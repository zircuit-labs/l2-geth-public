...
var (
	...
	// ZKTrieEnabled allows for use of zktrie and poseidon hashes
	// set via command-line flag `zktrie-enabled`
	ZKTrieEnabled atomic.Bool
)
...
var (
	...
	AllDevChainProtocolChanges = &ChainConfig{
		...
		TerminalTotalDifficulty:       big.NewInt(0),
		TerminalTotalDifficultyPassed: true,
		ZKTrieSwitchBlock:             big.NewInt(12),
	}
...
type ChainConfig struct {
	...
	ZKTrieSwitchBlock   *big.Int `json:"ZKTrieSwitchBlock,omitempty"`   // Block to fully switch on ZKTrie (nil = no fork, 0 = already on ZKTrie)
...
func (s ScrollConfig) String() string {
	...
	return fmt.Sprintf("{useZktrie: %v, maxTxPerBlock: %v, MaxTxPayloadBytesPerBlock: %v}",
		ZKTrieEnabled.Load(), maxTxPerBlock, maxTxPayloadBytesPerBlock)
}
...
// IsZKTrieSwitch returns whether num is either equal to the full zkTrie switch block or greater.
func (c *ChainConfig) IsZKTrieSwitch(num *big.Int) bool {
	return isBlockForked(c.ZKTrieSwitchBlock, num)
}
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	for _, cur := range []fork{
		...
		{name: "zkTrieSwitchBlock", block: c.ZKTrieSwitchBlock, optional: true},
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, headNumber *big.Int, headTimestamp uint64) *ConfigCompatError {
	...
	if isForkBlockIncompatible(c.ZKTrieSwitchBlock, newcfg.ZKTrieSwitchBlock, headNumber) {
		return newBlockCompatError("ZKTrie fork block", c.ZKTrieSwitchBlock, newcfg.ZKTrieSwitchBlock)
	}
