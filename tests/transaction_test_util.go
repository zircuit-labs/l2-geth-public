...
func (tt *TransactionTest) Run(config *params.ChainConfig) error {
	validateTx := func(rlpData hexutil.Bytes, signer types.Signer, isHomestead bool, isIstanbul bool) (*common.Address, *common.Hash, error) {
		...
		if rules.IsTenrec {
			...
			if err != nil {
				return &sender, &common.Hash{}, fmt.Errorf("Error calculating floor data gas.")
			}
			if tx.Gas() < floorDataGas {
				return &sender, &common.Hash{}, fmt.Errorf("%w: have %d, want %d", core.ErrFloorDataGas, tx.Gas(), floorDataGas)
			}
		}
