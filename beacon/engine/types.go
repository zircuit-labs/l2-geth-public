...
//go:generate go tool gencodec -type PayloadAttributes -field-override payloadAttributesMarshaling -out gen_blockparams.go
...
type PayloadAttributes struct {
	...
	// L1Info contains the L1 state information
...
//go:generate go tool gencodec -type ExecutableData -field-override executableDataMarshaling -out gen_ed.go
...
type RejectedPayloadMeta struct {
	TxsRejectedBySLS []common.Hash `json:"txsRejectedBySLS"`
	BlockFailedSLS   bool          `json:"blockFailedSLS"`
}
...
//go:generate go tool gencodec -type ExecutionPayloadEnvelope -field-override executionPayloadEnvelopeMarshaling -out gen_epe.go
...
type ForkChoiceResponse struct {
	...
	RejectedTransactions []common.Hash   `json:"RejectedTransactions"`
...
type ForkchoiceStateV1 struct {
	...
	// whether transactions should be checked and potentially rejected
	CheckTransactions bool          `json:"checkPayloadAttributes,omitempty"`
	L1Info            *types.L1Info `json:"l1Info,omitempty"`
}
...
func ExecutableDataToBlock(params ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash, requests [][]byte, bType types.BlockType) (*types.Block, error) {
	...
	if bType.HasOptimismWithdrawalsRoot(params.Timestamp) {
		if params.WithdrawalsRoot == nil {
			return nil, fmt.Errorf("attribute WithdrawalsRoot is required for Tenrec blocks")
		}
		if params.Withdrawals == nil || len(params.Withdrawals) > 0 {
			return nil, fmt.Errorf("expected non-nil empty withdrawals operation list in Tenrec, but got: %v", params.Withdrawals)
		}
	}
	if params.WithdrawalsRoot != nil {
		h := *params.WithdrawalsRoot // copy, avoid any sharing of memory
		withdrawalsRoot = &h
	...
	isTenrec := bType.IsTenrec(params.Timestamp)
	var requestsHash *common.Hash
	if requests != nil {
		if isTenrec && len(requests) > 0 {
			return nil, fmt.Errorf("requests should be empty for Tenrec blocks")
		}
		h := types.CalcRequestsHash(requests)
		requestsHash = &h
	} else if isTenrec {
		return nil, fmt.Errorf("requests must be an empty array for Tenrec blocks")
	}
...
func BlockToExecutableData(block *types.Block, fees *big.Int, sidecars []*types.BlobTxSidecar, requests [][]byte, l1Info *types.L1Info) *ExecutionPayloadEnvelope {
	data := &ExecutableData{
