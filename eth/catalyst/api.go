...
import (
	...
	"github.com/zircuit-labs/l2-geth/eth"
	...
	"github.com/zircuit-labs/l2-geth/params/forks"
	"github.com/zircuit-labs/l2-geth/rpc"
	"github.com/zircuit-labs/zkr-go-common/stores/pg"
	"github.com/zircuit-labs/zkr-go-common/xerrors/stacktrace"
)

//go:generate go tool mockgen -source api.go -destination mock_api.go -package catalyst

// Register adds the engine API to the full node.
func Register(stack *node.Node, backend *eth.Ethereum) error {
	log.Warn("Engine API enabled", "protocol", "eth")
	consensusAPI, err := NewConsensusAPI(backend)
	if err != nil {
		return err
	}
	...
	stack.RegisterLifecycle(consensusAPI)
	return nil
}
...
const (
	...
	// SLSTimeout is the max time allowed for performing SLS operations
	// in an FCU call. It is less than the block time so that the sequencer
	// can finish building block with subsequent api calls.
	SLSTimeout = 1800 * time.Millisecond
)
...
type ConsensusAPI struct {
	...
	refreshableGetter sls.RefreshableGetter // func to get sls refreshable configs
}

type (
	detectorManager interface {
		ShouldBeQuarantined(context.Context, *types.Transaction, slsCommon.DetectorType) (slsCommon.ManagerResult, error)
		Stop()
	}

	rejector interface {
		RecordRejectedTx(ctx context.Context, transaction *types.Transaction, detector, reason string, loss uint64) error
...
func NewConsensusAPI(eth *eth.Ethereum) (*ConsensusAPI, error) {
	...
	logger := slslog.NewWith("consensus_api", true)

	// SLS
	logger.With("enabled", eth.SLSConfig().IsEnabled()).Debug("Starting SLS on ConsensusAPI")
	refreshableGetter := sls.GetterOrStatic(eth.SLSConfig().Refreshable)
	api.refreshableGetter = refreshableGetter

	// Check if SLS is enabled.
	if eth.SLSConfig().IsEnabled() {

		api.signer = types.LatestSigner(api.eth.BlockChain().Config())

		slsDatabase, err := storage.NewStorage(ctx, eth.SLSConfig())
		if err != nil {
			logger.With("err", err).Warn("Can't connect to SLS storage")
			return nil, stacktrace.Wrap(err)
		}

		slsConfig := eth.SLSConfig()
		...
		detectorFactory := detector.NewFactory(slsCommonDetector.Dependencies[sls.Config, sls.RefreshableGetter]{
			...
			SLSConfig:             &slsConfig,
			GetRefreshableConfigs: api.refreshableGetter,
		})

		refreshInterval := sls.DefaultDetectorStatusRefreshInterval
		if slsConfig.DetectorStatusRefreshInterval > 0 {
			refreshInterval = time.Duration(slsConfig.DetectorStatusRefreshInterval)
		}
		...
		if err := detectorRegistry.Start(ctx); err != nil {
			logger.Warn("Failed to start detector registry", "err", err)
			return nil, stacktrace.Wrap(err)
		}

		trustVerifiers := []slsCommon.TrustVerifier{
			trustverifier.NewSystemTransactions(api.signer),
			trustverifier.NewTrustList(api.signer, slsDatabase),
		}

		api.detectorManager = sls.NewManager(api.refreshableGetter, detectorRegistry, trustVerifiers, slsDatabase, metricsCollector)

		api.rejector = sls.NewRejecter(slsDatabase, api.signer)
	}

	logger.With("enabled", eth.SLSConfig().IsEnabled()).Info("Started SLS on ConsensusAPI")
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if payloadAttributes != nil {
		...
		if update.CheckTransactions {
			slsCtx, cancel := context.WithTimeout(context.Background(), time.Duration(SLSTimeout))
			defer cancel()
			txs, err := api.ValidateTransactionsWithSLS(slsCtx, payloadAttributes.Transactions)
			if err != nil {
				return engine.STATUS_INVALID, engine.GenericServerError.With(err)
			}
			if len(txs) > 0 {
				return engine.ForkChoiceResponse{PayloadStatus: engine.PayloadStatusV1{Status: engine.SLSREJECTED}, RejectedTransactions: txs}, nil
			}
		}
	}
...
func (api *ConsensusAPI) ForkchoiceUpdatedV2(update engine.ForkchoiceStateV1, params *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if params != nil {
		...
		if update.CheckTransactions {
			slsCtx, cancel := context.WithTimeout(context.Background(), time.Duration(SLSTimeout))
			defer cancel()
			txs, err := api.ValidateTransactionsWithSLS(slsCtx, params.Transactions)
			if err != nil {
				return engine.STATUS_INVALID, engine.GenericServerError.With(err)
			}
			if len(txs) > 0 {
...
func (api *ConsensusAPI) ForkchoiceUpdatedV3(update engine.ForkchoiceStateV1, params *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if params != nil {
		// TODO(matt): according to https://github.com/zircuit-labs/execution-apis/pull/498,
		...
		if api.eth.BlockChain().Config().IsTenrec(params.Timestamp) {
			if params.BeaconRoot == nil {
				return engine.STATUS_INVALID, engine.InvalidParams.With(errors.New("missing beacon root"))
			}
		} else if params.BeaconRoot != nil {
			return engine.STATUS_INVALID, engine.InvalidParams.With(fmt.Errorf("invalid parentBeaconRoot, have %#x, expected nil", *params.BeaconRoot))
		}

		if !api.checkFork(params.Timestamp, forks.Cancun, forks.Prague, forks.Osaka) {
			return engine.STATUS_INVALID, engine.UnsupportedFork.With(errors.New("forkchoiceUpdatedV3 must only be called for cancun/prague/osaka payloads"))
		}
		if update.CheckTransactions {
			slsCtx, cancel := context.WithTimeout(context.Background(), time.Duration(SLSTimeout))
			defer cancel()
...
var (
	...
	ErrSLSDetection = errors.New("errors were returned when validating the transactions with SLS")
)
...
func (api *ConsensusAPI) ValidateTransactionsWithSLS(ctx context.Context, transactions [][]byte) ([]common.Hash, error) {
	logger := slslog.NewWith("consensus_api", true)
	logger.Debug("Validating transactions with SLS.")

	if !api.refreshableGetter().Enabled {
		logger.Debug("SLS is disabled. Skipping transactions validation.")
		return nil, nil
	}

	maxConcurrency := api.eth.SLSConfig().MaxConcurrency()

	logger.Debug("SLS max concurrency configuration", "maxConcurrency", maxConcurrency)

	errChan := make(chan error, len(transactions))
	txChan := make(chan transactionTask)
	rejectedTxsChan := make(chan common.Hash, len(transactions))

	var wg sync.WaitGroup

	api.startWorkers(ctx, logger, maxConcurrency, &wg, txChan, errChan, rejectedTxsChan)

	wg.Wait()
	close(rejectedTxsChan)
	close(errChan)

	rejectedTxs := api.collectRejectedTransactions(rejectedTxsChan)
	errs := api.collectErrors(errChan)

	if len(errs) > 0 {
		return nil, errors.Join(append(errs, ErrSLSDetection)...)
	}

	logger.Debug("Transactions scanned.", "rejectedTxs", len(rejectedTxs))

	return rejectedTxs, nil
}

func (api *ConsensusAPI) startWorkers(
	ctx context.Context,
	logger log.Logger,
	maxConcurrency int,
	wg *sync.WaitGroup,
	txChan <-chan transactionTask,
	errChan chan<- error,
	rejectedTxsChan chan<- common.Hash,
) {
	for i := range maxConcurrency {
		wg.Add(1)
		go func(workerID int) {
			defer wg.Done()
			api.worker(ctx, logger, workerID, txChan, errChan, rejectedTxsChan)
		}(i)
	}
}

func (api *ConsensusAPI) dispatchTransactions(ctx context.Context, transactions [][]byte, txChan chan<- transactionTask) []common.Hash {
	var leftOverTxs []common.Hash
	defer close(txChan)

	for i, rawTx := range transactions {
		select {
		case <-ctx.Done():
			log.Warn("timed out before all txs are sls checked", "txs missed", len(transactions)-i)

			for j := i; j < len(transactions); j++ {
				var tx types.Transaction
				if err := tx.UnmarshalBinary(transactions[j]); err != nil {
					log.Error("Invalid transaction binary", "error", err)
					continue
				}
				txHash := tx.Hash()
				log.Warn("tx missing sls check", "index", j, "hash", txHash)
				leftOverTxs = append(leftOverTxs, txHash)
			}
			return leftOverTxs
		default:
		}
		txChan <- transactionTask{index: i, rawTransaction: rawTx}
	}
	return leftOverTxs
}

func (api *ConsensusAPI) collectRejectedTransactions(rejectedTxsChan <-chan common.Hash) []common.Hash {
	var rejectedTxs []common.Hash
	for txHash := range rejectedTxsChan {
		rejectedTxs = append(rejectedTxs, txHash)
	}
	return rejectedTxs
}

func (api *ConsensusAPI) collectErrors(errChan <-chan error) []error {
	var errs []error
...
func (api *ConsensusAPI) worker(
	...
	for {
		select {
		case <-ctx.Done():
			logger.Info("Worker stopped by context", "workerID", id)
			return
		case task, ok := <-txChan:
			if !ok {
				logger.Debug("Tx channel closed, worker exiting", "workerID", id)
				return
			}

			txCtx, cancel := context.WithTimeout(ctx, api.eth.SLSConfig().DepositTxTimeout())
			err := api.processTransaction(txCtx, logger, task.index, task.rawTransaction, rejectedTxsChan)
			cancel()
...
func (api *ConsensusAPI) processTransaction(ctx context.Context, logger log.Logger, index int, rawTransaction []byte, rejectedTxsChan chan<- common.Hash) error {
	logger = logger.With("index", index)

	var tx types.Transaction
	if err := tx.UnmarshalBinary(rawTransaction); err != nil {
		logger.Warn("Invalid transaction binary", "error", err)
		return fmt.Errorf("transaction %d is not valid: %v", index, err)
	}

	logger = logger.With("tx_hash", tx.Hash().String())

	if tx.To() != nil {
		logger = logger.With("to", tx.To().String())
	}

	sender, err := api.signer.Sender(&tx)
	if err != nil {
		logger.Warn("Error finding transaction sender", "error", err)
		return fmt.Errorf("can't find transaction sender %d: %v", index, err)
	}

	logger = logger.With("sender", sender.String())

	if types.ProtocolTx(index, &tx, sender) {
		logger.Debug("System transaction identified, skipping SLS")
		return nil
	}

	detectorResult, err := api.detectorManager.ShouldBeQuarantined(ctx, &tx, slsCommon.DetectorTypeForkChoice)
	if err != nil {
		logger.Warn("Error scanning transaction", "error", err)

		return fmt.Errorf("can't scan transaction %d: %v", index, err)
	}

	if detectorResult.ShouldBeQuarantined {
		rejectedTxsChan <- tx.Hash()
		logger.Info("SLS - Transaction was flagged.", "index", index, "hash", tx.Hash())

		if err := api.rejector.RecordRejectedTx(context.Background(), &tx, detectorResult.Detectors, detectorResult.Reasons, detectorResult.Loss); err != nil {
			logger.Warn("Error recording rejected transaction", "error", err)
			return fmt.Errorf("can't record rejected transaction %s: %v", tx.Hash().String(), err)
		}
	}
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes, payloadVersion engine.PayloadVersion, simulatorMode bool) (engine.ForkChoiceResponse, error) {
	...
	valid := func(id *engine.PayloadID) engine.ForkChoiceResponse {
		merkleRoot := ""
		if api.refreshableGetter != nil {
			if sls.GetEnableStray(api.refreshableGetter) {
				// call zrc api to get merkleroot
			}
		}
	...
	if payloadAttributes != nil {
		...
		if cfg.Optimism != nil {
			if payloadAttributes.GasLimit == nil {
				return engine.STATUS_INVALID, engine.InvalidPayloadAttributes.With(consensus.ErrMissingGasLimit)
			}
		...
		for i, otx := range payloadAttributes.Transactions {
			...
			if !tx.IsDepositTx() {
				totalPayloadSize += uint64(len(otx))
			}
		}

		// block validation for transaction/block limits are no longer done.
		// the sequencer dictates the size limits to allow more flexibility
		if !cfg.IsAlfie(block.Time()) {
			if !cfg.Scroll.IsValidTxCount(len(payloadAttributes.Transactions)) {
				return engine.STATUS_INVALID, engine.InvalidPayloadAttributes.With(core.ErrInvalidBlockPayloadSize)
			}

			// Check if block payload size is smaller than the max size
			if !cfg.Scroll.IsValidBlockSize(totalPayloadSize) {
				return engine.STATUS_INVALID, engine.InvalidPayloadAttributes.With(errors.New("exceeding max payload size"))
			}
		}
		...
		args := &miner.BuildPayloadArgs{
			...
			L1Info:            payloadAttributes.L1Info,
			CheckTransactions: update.CheckTransactions,
		}
		id := args.Id()
...
func (api *ConsensusAPI) NewPayloadV2(params engine.ExecutableData) (engine.PayloadStatusV1, error) {
	...
	if api.eth.BlockChain().Config().LatestFork(params.Timestamp) == forks.Shanghai || api.eth.BlockChain().Config().LatestFork(params.Timestamp) == forks.Prague {
		if params.Withdrawals == nil {
...
func (api *ConsensusAPI) NewPayloadV3(params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash) (engine.PayloadStatusV1, error) {
	...
	if beaconRoot != nil {
		return engine.PayloadStatusV1{Status: engine.INVALID}, engine.InvalidParams.With(fmt.Errorf("invalid parentBeaconRoot, have %#x, expected nil", *beaconRoot))
	}
	...
	return api.newPayload(params, versionedHashes, nil, beaconRoot)
}
...
func (api *ConsensusAPI) NewPayloadV4(params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash, executionRequests []hexutil.Bytes) (engine.PayloadStatusV1, error) {
	...
	if !api.checkFork(params.Timestamp, forks.Prague, forks.Osaka) {
		return engine.PayloadStatusV1{Status: engine.INVALID}, engine.UnsupportedFork.With(errors.New("newPayloadV4 must only be called for prague/osaka payloads"))
	}

	if api.eth.BlockChain().Config().IsTenrec(params.Timestamp) && params.WithdrawalsRoot == nil {
		return engine.PayloadStatusV1{Status: engine.INVALID}, engine.InvalidParams.With(errors.New("nil withdrawalsRoot post-tenrec"))
	}
	...
	return api.newPayload(params, versionedHashes, requests, beaconRoot)
}

func (api *ConsensusAPI) newPayload(params engine.ExecutableData, versionedHashes []common.Hash, requests [][]byte, beaconRoot *common.Hash) (engine.PayloadStatusV1, error) {
	...
	if _, err := api.eth.BlockChain().InsertBlockWithoutSetHead(block, false); err != nil {
	...
	rawdb.WriteL1Info(api.eth.ChainDb(), block.Hash(), params.L1Info)
...
func (api *ConsensusAPI) heartbeat() {
	...
	for {
		...
		case <-api.stopCh:
			log.Debug("heartbeat goroutine shutting down")
			return
		}
	}
}
...
func (api *ConsensusAPI) Start() error {
	log.Debug("starting consensusAPI")
	return nil
}

func (api *ConsensusAPI) Stop() error {
	if api.stopCh != nil {
		close(api.stopCh)
	}
	if api.detectorManager != nil {
		api.detectorManager.Stop()
	}
	return nil
}
