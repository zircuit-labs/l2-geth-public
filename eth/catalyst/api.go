...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/params/forks"
	"github.com/zircuit-labs/l2-geth-public/rpc"
)

//go:generate mockgen -source api.go -destination mock_api.go -package catalyst
...
type (
	detectorManager interface {
		ShouldBeQuarantined(context.Context, *types.Transaction) (sls.ManagerResult, error)
		Stop()
	}
...
func NewConsensusAPI(eth *eth.Ethereum) (*ConsensusAPI, error) {
	...
	// SLS
	logger.With("enabled", eth.SLSConfig().Enabled).Debug("Starting SLS on ConsensusAPI")

	// Check if SLS is enabled.
	if eth.SLSConfig().Enabled {

		slsDatabase, err := storage.NewStorage(context.TODO(), storage.Config{DSN: eth.SLSConfig().DSN})
		if err != nil {
		...
		//TODO: close the background go-routine
		detectors := [][]sls.Detector{
			0: {},
		}

		trustVerifiers := []sls.TrustVerifier{}

		api.detectorManager = sls.NewManager(detectors, trustVerifiers, slsDatabase)

		api.rejector = sls.NewRejecter(slsDatabase, api.signer)
	}

	logger.With("enabled", eth.SLSConfig().Enabled).Info("Started SLS on ConsensusAPI")
...
func (api *ConsensusAPI) ValidateTransactionsWithSLS(ctx context.Context, transactions [][]byte) ([]common.Hash, error) {
	...
	if !api.eth.SLSConfig().Enabled {
		logger.Debug("SLS is disabled. Skipping transactions validation.")
		return nil, nil
	}

	maxConcurrency := 1
	if api.eth.SLSConfig().MaxConcurrencyPool > 1 {
		maxConcurrency = api.eth.SLSConfig().MaxConcurrencyPool
	}
	...
	leftOvers := api.dispatchTransactions(ctx, transactions, txChan)
	...
	if len(leftOvers) > 0 && !api.eth.SLSConfig().AssumeInnocenceOnError {
		// we are rejecting txs if context timeout.
		// if we don't assume innocence, consider increasing the timeout
		logger.Warn("txs that missed sls checks due to ctx timeout are rejected")
		rejectedTxs = append(rejectedTxs, leftOvers...)
	}
...
func (api *ConsensusAPI) startWorkers(ctx context.Context, logger log.Logger, maxConcurrency int, wg *sync.WaitGroup, txChan <-chan transactionTask, errChan chan<- error, rejectedTxsChan chan<- common.Hash) {
	for i := 0; i < maxConcurrency; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			api.worker(ctx, logger, txChan, errChan, rejectedTxsChan)
		}()
	}
}
...
func (api *ConsensusAPI) worker(ctx context.Context, logger log.Logger, txChan <-chan transactionTask, errChan chan<- error, rejectedTxsChan chan<- common.Hash) {
	for task := range txChan {
		txCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
		if err := api.processTransaction(txCtx, logger, task.index, task.rawTransaction, rejectedTxsChan); err != nil {
			errChan <- err
		}
		cancel()
		select {
		case <-ctx.Done():
			// just for logging. The loop will exit when txChan is closed
			log.Info("context timed out. Returning worker")
		default:
		}
	}
}
...
func (api *ConsensusAPI) processTransaction(ctx context.Context, logger log.Logger, index int, rawTransaction []byte, rejectedTxsChan chan<- common.Hash) error {
	...
	detectorResult, err := api.detectorManager.ShouldBeQuarantined(ctx, &tx)
	if err != nil {
		logger.Warn("Error scanning transaction", "error", err)
		if api.eth.SLSConfig().AssumeInnocenceOnError {
			logger.Info("Assuming transaction is innocent due to error")
			return nil
		}
