...
import (
	...
	beaconConsensus "github.com/zircuit-labs/l2-geth-public/consensus/beacon"
	...
	"github.com/zircuit-labs/l2-geth-public/core/txpool/legacypool"
	...
	"github.com/zircuit-labs/l2-geth-public/eth/downloader"
	...
	"github.com/zircuit-labs/l2-geth-public/p2p"
...
func TestEth2AssembleBlock(t *testing.T) {
	t.Skip("skipping flaky test") // TODO: test is flaky
	...
	if err != nil {
		t.Fatalf("error creating consensus API, err=%v", err)
	}
...
func startEthServiceWithSLS(t *testing.T, genesis *core.Genesis, blocks []*types.Block, SLSConf sls.Config) (*node.Node, *eth.Ethereum) {
	ethcfg := &ethconfig.Config{
		Genesis: genesis, SyncMode: downloader.FullSync, TrieTimeout: time.Minute, TrieDirtyCache: 256, TrieCleanCache: 256,
		TxPool: legacypool.Config{},
		CCCConfig: circuitcapacitychecker.Config{
			Enabled:       true,
			CheckDeposits: true,
		},
		SLSConfig: SLSConf,
...
func TestTrickRemoteBlockCache(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("error creating consensus APIA, err=%v", err)
	}
...
func createDummyTxs(signer types.Signer, n int, gasPrice *big.Int) ([][]byte, error) {
	var (
		key, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		txs    = make([][]byte, n)
	)
	for i := 0; i < n; i++ {
		tx, err := types.SignTx(types.NewTransaction(uint64(10+i), common.Address{0x00}, big.NewInt(1000), params.TxGas, gasPrice, nil), signer, key)
		if err != nil {
			return nil, err
		}
...
func createDummyInvalidTx(signer types.Signer, n int, gasPrice *big.Int) ([][]byte, error) {
	var (
		key, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		txs    = make([][]byte, n)
	)
	for i := 0; i < n; i++ {
		//invalid tx with 0 nonce
		tx, err := types.SignTx(types.NewTransaction(0, common.Address{0x00}, big.NewInt(1000), params.TxGas, gasPrice, nil), signer, key)
		if err != nil {
			return nil, err
		}
		txBinary, err := tx.MarshalBinary()
...
func TestForkchoiceUpdatedV2WithDepositTransactions(t *testing.T) {
	...
	txs, err := createDummyTxs(types.LatestSigner(genesis.Config), 1, blocks[len(blocks)-1].Header().BaseFee)
	if err != nil {
		t.Fatalf("error creating txs, err=%v", err)
	}
...
func TestGetPayloadWithDepositTransactions(t *testing.T) {
	genesis, blocks := generateMergeChain(10, true)
	// Set shanghai time to last block + 5 seconds (first post-merge block)
	time := blocks[len(blocks)-1].Time() + 5
	genesis.Config.ShanghaiTime = &time
	...
	type test struct {
		...
		failCCC         bool
		wantErr         bool
		rejectedPayload bool
	}
	tests := []test{
		{
			name:            "With one transaction only",
			numOfTxs:        1,
			failCCC:         false,
			wantErr:         false,
		...
		{
			name:            "With multiple transactions but CCC fails",
			numOfTxs:        3,
			failCCC:         true,
			wantErr:         false,
		...
		{
			name:            "With same failed txs twice",
			numOfTxs:        3,
			failCCC:         false,
			wantErr:         true,
			rejectedPayload: false,
	...
	for _, tt := range tests {
		if tt.failCCC {
			// Schedule an error for the first tx execution in CCC
			ethservice.Miner().Worker().GetCCC().ScheduleError(1, circuitcapacitychecker.ErrBlockRowConsumptionOverflow)
			fmt.Println("gonna fail ccc")
		}
		txs, err := createDummyTxs(types.LatestSigner(genesis.Config), tt.numOfTxs, blocks[len(blocks)-1].Header().BaseFee)
		if err != nil {
			t.Fatalf("error creating txs, err=%v", err)
		}
		blockParams := engine.PayloadAttributes{
		...
		//get the payload
		execEnv, err := api.GetPayloadV2(*resp.PayloadID)
		if (err != nil) != tt.wantErr {
			t.Fatalf("GetPayloadV2() error = %v, wantErr %v", err, tt.wantErr)
		}
		//verify rejected transactions in the envelope
		if (execEnv != nil && execEnv.RejectedPayloadM != nil) != tt.rejectedPayload {
			t.Fatalf("unexpected rejected transactions, got: %v", execEnv.RejectedPayloadM)
		}
		// verify the execution error
		if (execEnv != nil && execEnv.ExecutionErr != nil) != tt.rejectedPayload {
			t.Fatalf("unexpected execution error, got: %v", execEnv.ExecutionErr)
		}

	}
}

func TestGetPayloadWithInvalidTransactions(t *testing.T) {
	genesis, blocks := generateMergeChain(10, true)
	// Set shanghai time to last block + 5 seconds (first post-merge block)
	time := blocks[len(blocks)-1].Time() + 5
	genesis.Config.ShanghaiTime = &time
	...
	tests := []test{
		...
		{
			name:     "With 2 transactions where one is invalid",
			numOfTxs: 2,
			ValidTxs: 1,
			wantErr:  true,
		},
	...
	for _, tt := range tests {
		...
		txs, err := createDummyTxs(types.LatestSigner(genesis.Config), tt.ValidTxs, blocks[len(blocks)-1].Header().BaseFee)
		if err != nil {
			t.Fatalf("error creating txs, err=%v", err)
		}

		invalidTxs, err := createDummyInvalidTx(types.LatestSigner(genesis.Config), tt.numOfTxs-tt.ValidTxs, blocks[len(blocks)-1].Header().BaseFee)
		if err != nil {
			t.Fatalf("error creating txs, err=%v", err)
		}
		txs = append(txs, invalidTxs...)
		...
		//get the payload
		_, err = api.GetPayloadV2(*resp.PayloadID)
		if (err != nil) != tt.wantErr {
			t.Fatalf("GetPayloadV2() error = %v, wantErr %v", err, tt.wantErr)
		}
	}
...
func TestBlockToPayloadWithBlobs(t *testing.T) {
	...
	envelope := engine.BlockToExecutableData(block, nil, sidecars, &types.L1Info{})
	var want int
	for _, tx := range txs {
...
func TestIsSystemTransaction(t *testing.T) {
	...
	someAddress := common.HexToAddress("0x1111111111111111111111111111111111111111")
	...
	tests := []struct {
	...
	}{
		{
			...
			index:  0,
			tx:     createTransaction(&types.L1BlockAddr),
			sender: systemAddress,
			want:   true,
		},
		{
			name:   "Non-system transaction - different index",
			index:  1,
			tx:     createTransaction(&types.L1BlockAddr),
			sender: systemAddress,
			want:   false,
		...
		{
			name:   "Non-system transaction - different sender",
			index:  0,
			tx:     createTransaction(&types.L1BlockAddr),
			sender: someAddress,
			want:   false,
		...
		{
			name:   "Non-system transaction - different recipient",
			index:  0,
			tx:     createTransaction(&someAddress),
			sender: systemAddress,
			want:   false,
		},
		{
			name:   "Non-system transaction - no recipient",
			index:  0,
			tx:     createTransaction(nil),
			sender: systemAddress,
			want:   false,
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := types.ProtocolTx(tt.index, tt.tx, tt.sender)
			assert.Equal(t, tt.want, got)
		})
	}
}

func createTransaction(to *common.Address) *types.Transaction {
...
func TestDispatchTransactions(t *testing.T) {
	t.Parallel()
	...
	timedOutCtx, cancel := context.WithDeadline(context.Background(), time.Now().Add(-time.Second))
	defer cancel()
	...
	tests := []struct {
		...
		expectedDispatchedTasks int
		result                  []transactionTask
	}{
		{
			name:                    "All transactions dispatched",
			ctx:                     context.Background(),
			txs:                     transactions,
			txChan:                  make(chan transactionTask, 10),
			expectedDispatchedTasks: 3,
			result: []transactionTask{
				{index: 0, rawTransaction: transactions[0]},
				{index: 1, rawTransaction: transactions[1]},
				{index: 2, rawTransaction: transactions[2]},
			},
		},
		{
			name:                    "No transactions dispatched",
			ctx:                     timedOutCtx,
			txs:                     transactions,
			txChan:                  make(chan transactionTask, 10),
			expectedDispatchedTasks: 0,
			result:                  []transactionTask{},
		},
	}
	for _, tc := range tests {

		api.dispatchTransactions(tc.ctx, tc.txs, tc.txChan)
		// Collect the dispatched tasks
		actualTasks := []transactionTask{}
		for task := range tc.txChan {
			actualTasks = append(actualTasks, task)
		}

		assert.Equal(t, tc.expectedDispatchedTasks, len(actualTasks))
		assert.Equal(t, tc.result, actualTasks)
	}
}

func TestCollectRejectedTransactions(t *testing.T) {
	t.Parallel()

	hash1 := common.HexToHash("0x123")
	hash2 := common.HexToHash("0x456")
	hash3 := common.HexToHash("0x789")
	...
	tests := []struct {
		...
		{
			name:     "No rejected transactions",
			input:    []common.Hash{},
			expected: nil,
		},
		{
			name:     "One rejected transaction",
			input:    []common.Hash{hash1},
			expected: []common.Hash{hash1},
		},
		{
			name: "Multiple rejected transactions",
			input: []common.Hash{
				hash1,
				hash2,
				hash3,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			rejectedTxsChan := make(chan common.Hash, len(tt.input))

			for _, txHash := range tt.input {
				rejectedTxsChan <- txHash
			}
			...
			assert.Equal(t, tt.expected, rejectedTxs)
		})
	}
}

func TestValidateTransactionsWithSLS(t *testing.T) {
	genesis, blocks := generateMergeChain(10, true)
	// Set shanghai time to last block + 5 seconds (first post-merge block)
	tm := blocks[len(blocks)-1].Time() + 5
	genesis.Config.ShanghaiTime = &tm

	slsConf := sls.Config{
		Enabled:                true,
		QuarantineDuration:     duration.Duration(1 * time.Second),
		AssumeInnocenceOnError: true,
	}
	n, ethservice := startEthServiceWithSLS(t, genesis, blocks, slsConf)
	ethservice.Merger().ReachTTD()
	defer n.Close()
	...
	txs, err := createDummyTxs(types.LatestSigner(genesis.Config), 5, blocks[len(blocks)-1].Header().BaseFee)
	if err != nil {
		t.Fatalf("error creating txs, err=%v", err)
	}
	...
	for _, tc := range tests {
		...
		rejected, err := api.ValidateTransactionsWithSLS(tc.ctx, tc.txs)

		if err != nil != tc.wantErr {
			t.Fatalf("ValidateTransactionsWithSLS() error = %v, wantErr %v", err, tc.wantErr)
		}

		if len(rejected) != tc.rejectedTxs {
			t.Fatalf("ValidateTransactionsWithSLS() rejectedTxs = %v, want %v", len(rejected), tc.rejectedTxs)
		}
	}
...
func (m mockSigner) Sender(*types.Transaction) (common.Address, error) {
...
func TestProcessTransaction(t *testing.T) {
	...
	type args struct {
		index            int
		rawTransaction   []byte
		setup            func(mockDetectorManager *MockdetectorManager, mockRejector *Mockrejector) *mockSigner
		expectedError    bool
		expectedTxRecord int
		slsConfig        sls.Config
	}
	tests := []struct {
		name string
		args args
		...
		{
			name: "Error finding transaction sender",
			args: args{
				rawTransaction: createTransactionBytes(t, &someAddress),
				setup: func(mockDetectorManager *MockdetectorManager, mockRejector *Mockrejector) *mockSigner {
					return &mockSigner{err: errors.New("can't get signer")}
				},
				expectedError: true,
			},
		},
		{
			name: "System transaction, skipping SLS",
			args: args{
				index:          0,
				rawTransaction: createTransactionBytes(t, &types.L1BlockAddr),
				setup: func(mockDetectorManager *MockdetectorManager, mockRejector *Mockrejector) *mockSigner {
					return &mockSigner{sender: systemAddress}
				},
				expectedError: false,
			},
		},
		{
			name: "Error scanning transaction",
			args: args{
				index:          1,
				rawTransaction: createTransactionBytes(t, &someAddress),
				slsConfig: sls.Config{
					AssumeInnocenceOnError: false,
				},
				setup: func(mockDetectorManager *MockdetectorManager, mockRejector *Mockrejector) *mockSigner {
					mockDetectorManager.EXPECT().ShouldBeQuarantined(ctx, gomock.Any()).Return(noResult, errors.New("error scanning transaction"))
					return &mockSigner{sender: otherAddress}
				},
				expectedError: true,
			},
		},
		{
			name: "Assume innocence on error",
			args: args{
				index:          1,
				rawTransaction: createTransactionBytes(t, &someAddress),
				slsConfig: sls.Config{
		...
		{
			name: "Can't record rejected tx",
			args: args{
				index:          1,
				rawTransaction: createTransactionBytes(t, &someAddress),
				setup: func(mockDetectorManager *MockdetectorManager, mockRejector *Mockrejector) *mockSigner {
					mockDetectorManager.EXPECT().ShouldBeQuarantined(ctx, gomock.Any()).Return(sls.ManagerResult{
						ShouldBeQuarantined: true,
						Detectors:           detectors,
						Reasons:             reasons,
						Loss:                loss,
					}, nil)
					mockRejector.EXPECT().RecordRejectedTx(gomock.Any(), gomock.Any(), detectors, reasons, loss).Return(errors.New("can't record"))
					return &mockSigner{sender: otherAddress}
				},
				expectedError:    true,
				expectedTxRecord: 1,
		...
		},
		{
			...
			args: args{
				...
				setup: func(mockDetectorManager *MockdetectorManager, mockRejector *Mockrejector) *mockSigner {
					...
					mockRejector.EXPECT().RecordRejectedTx(gomock.Any(), gomock.Any(), detectors, reasons, loss).Return(nil)
					return &mockSigner{sender: otherAddress}
				},
				expectedError:    false,
				expectedTxRecord: 1,
	...
	for _, tt := range tests {
		tt := tt
		t.Run(tt.name, func(t *testing.T) {
			mockDetectorManager := NewMockdetectorManager(ctrl)
			mockRejector := NewMockrejector(ctrl)
			mockSigner := tt.args.setup(mockDetectorManager, mockRejector)

			api := &ConsensusAPI{
				detectorManager: mockDetectorManager,
				rejector:        mockRejector,
				signer:          mockSigner,
			}

			api.eth, err = eth.New(n, &ethconfig.Config{
				SLSConfig: tt.args.slsConfig,
			})
			assert.NoError(t, err)

			rejectedTxsChan := make(chan<- common.Hash, 10)
			err = api.processTransaction(ctx, log.New(), tt.args.index, tt.args.rawTransaction, rejectedTxsChan)
			if (err != nil) != tt.args.expectedError {
				t.Errorf("processTransaction() error = %v, wantErr %v", err, tt.args.expectedError)
			}
			assert.Equal(t, tt.args.expectedTxRecord, len(rejectedTxsChan))
		})
	}
...
func createTransactionBytes(t *testing.T, to *common.Address) []byte {
	tx := createTransaction(to)
