...
import (
	...
	"github.com/zircuit-labs/l2-geth/accounts/abi/bind"
	...
	"github.com/zircuit-labs/l2-geth/crypto/kzg4844"
	"github.com/zircuit-labs/l2-geth/eth"
	"github.com/zircuit-labs/l2-geth/eth/ethconfig"
	"github.com/zircuit-labs/l2-geth/ethclient"
...
func TestMain(m *testing.M) {
	if _, err := sls.InitReader("", 0); err != nil {
		l.Fatalf("failed to init sls config reader: %v", err)
	}
	os.Exit(m.Run())
}
...
func setupBlocks(t *testing.T, ethservice *eth.Ethereum, n int, parent *types.Header, callback func(parent *types.Header), withdrawals [][]*types.Withdrawal, beaconRoots []common.Hash) []*types.Header {
	api, _ := NewConsensusAPI(ethservice)
	...
	for i := range n {
		...
		execResp, err := api.newPayload(*envelope.ExecutionPayload, []common.Hash{}, envelope.Requests, h)
		if err != nil {
	...
	for i := range 10 {
		...
		for i := 0; ; i++ {
			...
			if len(payload.ExecutionPayload.Transactions) > 0 {
				fmt.Println("len", len(payload.ExecutionPayload.Transactions))
				break
			}
		...
		parent = ethservice.BlockChain().CurrentBlock()
		if _, err := bind.WaitMined(context.Background(), ethclient.NewClient(n.Attach()), tx); err != nil {
...
func setupBodies(t *testing.T) (*node.Node, *eth.Ethereum, []*types.Block) {
	...
	callback := func(parent *types.Header) {
		...
		ethservice.TxPool().Add([]*types.Transaction{tx1}, true, false)
		ethservice.TxPool().Add([]*types.Transaction{tx2}, true, false)
	}
	...
	// Create the blocks.
	newHeaders := setupBlocks(t, ethservice, 10, parent, callback, withdrawals, nil)
...
func TestBlockToPayloadWithBlobs(t *testing.T) {
	...
	envelope := engine.BlockToExecutableData(block, nil, sidecars, nil, &types.L1Info{})
	var want int
	for _, tx := range txs {
...
func TestParentBeaconBlockRoot(t *testing.T) {
	t.Skip("Current circuits' implementation expects `parentBeaconRoot` always to be null, but it's kinda tricky to update rlp testdata not to contain this field")
