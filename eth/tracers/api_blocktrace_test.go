...
//type testTracerWrapper struct {
//	cccHelper *circuitcapacitychecker.CCCHelper
//}
//
//// NewTracerWrapper TracerWrapper creates a new TracerWrapper
//func newTestTracerWrapper(b circuitcapacitychecker.MiniBlockChain) *testTracerWrapper {
//	return &testTracerWrapper{
...
//func (tw *testTracerWrapper) CreateTraceEnvAndGetBlockTrace(
...
//	parent *types.Block, block *types.Block, commitAfterApply bool,
//) (*types.BlockTrace, error) {
//	traceEnv, err := tracing.CreateTraceEnv(chainConfig, chainContext, engine, chaindb, statedb, parent, block, commitAfterApply)
//	if err != nil {
//		return nil, err
//	}
//
...
//}
//
//func (tw *testTracerWrapper) GetCodesAndProofs(
...
//	assert.NoError(t, err)
//
//	scrollTracerWrapper := newTestTracerWrapper(backend.chain)
//	api := NewAPI(backend, scrollTracerWrapper)
//	// get trace
//	hash := block.Hash()
//	blockTrace, err := api.DebugBlockTrace(context.Background(), rpc.BlockNumberOrHash{
//		BlockHash: &hash,
//	}, nil)
//	assert.NoError(t, err)
//
//	// check chain status
//	checkChainAndProof(t, backend, parent, block, blockTrace.BlockTrace)
//
//	// check txs.
//	checkTxs(t, block.Transactions(), blockTrace.BlockTrace.Transactions)
//
//	txTraces, err := api.TraceBlockByNumber(context.Background(), 2, nil)
//	assert.NoError(t, err)
//
//	// check executionResults
//	checkStructLogs(t, txTraces, blockTrace.BlockTrace.ExecutionResults)
//
//	// check coinbase
//	checkCoinbase(t, backend, blockTrace.BlockTrace.Coinbase)
//}
//
...
//	assert.Equal(t, len(expect), len(actual))
//	for i, val := range expect {
//		trace1 := types.ExecutionResult{}
//		err := json.Unmarshal(val.Result.(json.RawMessage), &trace1)
//		assert.NoError(t, err)
//
//		trace2 := actual[i]
