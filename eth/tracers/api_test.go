...
func (m *mockHistoricalBackend) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (any, error) {
	ret := m.Mock.MethodCalled("TraceTransaction", hash, config)
	return ret[0], *ret[1].(*error)
}

func (m *mockHistoricalBackend) ExpectTraceTransaction(hash common.Hash, config *TraceConfig, out any, err error) {
...
func (b *testBackend) GetChainTransactions(ctx context.Context, number rpc.BlockNumber) []*types.Transaction {
	var (
		txs []*types.Transaction
		cur = number
	)
	for {
		curBlock, err := b.BlockByNumber(ctx, cur)
		if curBlock == nil || err != nil {
			break
		}
		txs = append(txs, curBlock.Transactions()...)
		cur++
	}
	return txs
}

func (b *testBackend) FilterTxsBySender(sender common.Address, signer types.Signer, txs []*types.Transaction) []*types.Transaction {
	var result []*types.Transaction

	for _, tx := range txs {
		from, _ := types.Sender(signer, tx)
		if from.Cmp(sender) == 0 {
...
func TestTraceCall(t *testing.T) {
	...
	testSuite := []struct {
		...
		},
		{
			...
			expectErr: nil,
			expect: ` {"gas":53018,"failed":false,"returnValue":"0x","structLogs":[null,null,
...
func TestTraceTransactionHistorical(t *testing.T) {
	...
	// marshalling nil, so later it is possible to successfully compare
	// empty marshalled callTrace
	res, err := json.Marshal(nil)
	if err != nil {
		t.Errorf("Failed to marshal nil %v", err)
	}
...
func TestFindCompleteExclusionList(t *testing.T) {
	...
	genesis := &core.Genesis{
		...
		Alloc: types.GenesisAlloc{
			...
			accounts[1].addr: {Balance: big.NewInt(params.Ether)},
			accounts[2].addr: {Balance: big.NewInt(params.Ether)},
			accounts[3].addr: {Balance: big.NewInt(10000 * params.GWei)}, // account[3] only has amount to pay for the gas
	...
	var (
		...
		txsInBlockPerAccount = 10
		transferAmount       = big.NewInt(10000 * params.GWei)
	)
	backend := newTestBackend(t, totalBlocks, genesis, func(i int, b *core.BlockGen) {
		// Transfer from account[0] to account[1]
		for range txsInBlockPerAccount {
			tx, _ := types.SignTx(types.NewTransaction(nonces[0], accounts[1].addr, transferAmount, params.TxGas, b.BaseFee(), nil), signer, accounts[0].key)
			b.AddTx(tx)
			nonces[0] += 1
		}
		// Transfer from account[1] to account[3]
		for range txsInBlockPerAccount {
			tx, _ := types.SignTx(types.NewTransaction(nonces[1], accounts[3].addr, transferAmount, params.TxGas, b.BaseFee(), nil), signer, accounts[1].key)
			b.AddTx(tx)
			nonces[1] += 1
		}
		// Transfer from account[3] to account[2] in first 10 blocks only
		// account[3] is empty. The transation shall be rejected if transfers to account[3] are removed first
		if i < 10 {
			tx, _ := types.SignTx(types.NewTransaction(nonces[3], accounts[2].addr, transferAmount, params.TxGas, b.BaseFee(), nil), signer, accounts[3].key)
			b.AddTx(tx)
			nonces[3] += 1
		}
	})
	...
	cases := []struct {
		...
		senderToExclude        Account
		senderTxIndToExclude   int
		expectedCompExclusions int
		addDuplicateTx         bool
		addNonExistantTx       bool
		expectErr              bool
	}{
		{
			name:                 "Removing 1st tx of the first account",
			start:                1,
			senderToExclude:      accounts[0],
			senderTxIndToExclude: 0,
			// shall only remove all txs of the first account
			// note that txs from 2nd account donot depend on txs from 1st account
			expectedCompExclusions: txsInBlockPerAccount * totalBlocks,
			addDuplicateTx:         false,
			expectErr:              false,
		},
		{
			name:                 "Add duplicate tx of the first account",
			start:                1,
			senderToExclude:      accounts[0],
			senderTxIndToExclude: 0,
			addDuplicateTx:       true,
		...
		{
			name:                 "Add non-existent tx",
			start:                1,
			senderToExclude:      accounts[0],
			senderTxIndToExclude: 0,
			addNonExistantTx:     true,
			expectErr:            true,
		},
		{
			name:                   "Removing first tx starting at block 5",
			start:                  5,
			senderToExclude:        accounts[0],
			senderTxIndToExclude:   0, // first tx encountered from block 5 onwards
			expectedCompExclusions: txsInBlockPerAccount * (totalBlocks - 5 + 1),
			addDuplicateTx:         false,
			expectErr:              false,
		},
		{
			name:                   "Removing first tx starting at block 50",
			start:                  50,
			senderToExclude:        accounts[0],
			senderTxIndToExclude:   0, // first tx encountered from block 50 onwards
			expectedCompExclusions: txsInBlockPerAccount * (totalBlocks - 50 + 1),
			addDuplicateTx:         false,
			expectErr:              false,
		},
		{
			name:                 "Removing first tx from second account",
			start:                1,
			senderToExclude:      accounts[1],
			senderTxIndToExclude: 0,
			// removing txs from second account make txs from 3rd account low in funds
			expectedCompExclusions: txsInBlockPerAccount*totalBlocks + 10, // 10 txs from 3rd account
			addDuplicateTx:         false,
			expectErr:              false,
		},
		{
			name:                 "Starting at block 0",
			start:                0,
			senderToExclude:      accounts[0],
			senderTxIndToExclude: 0,
			// genesis doesn't have a parent block to generate state from
			addDuplicateTx: false,
			expectErr:      true,
		},
		{
			name:                 "End block is less than start block",
			start:                10,
			end:                  9,
			senderToExclude:      accounts[0],
			senderTxIndToExclude: 0,
	...
	for _, c := range cases {
		...
		txs := backend.GetChainTransactions(context.Background(), rpc.BlockNumber(c.start))
		if len(txs) == 0 {
			t.Fatalf("no txs found in the chain")
		}
		txsFromExcludedSender := backend.FilterTxsBySender(c.senderToExclude.addr, signer, txs)
		initialExclusions := []common.Hash{txsFromExcludedSender[c.senderTxIndToExclude].Hash()}
		if c.addDuplicateTx {
			initialExclusions = append(initialExclusions, txsFromExcludedSender[c.senderTxIndToExclude].Hash())
		}
		if c.addNonExistantTx {
			initialExclusions = append(initialExclusions, common.HexToHash("0x1"))
		}
		if c.end != 0 {
			endBlockNum = &c.end
		}

		addr := c.senderToExclude.addr
		type account struct {
			Balance *big.Int
			Nonce   uint64
			Code    []byte
		...
		stateBefore, err := backend.chain.State()
		if err != nil {
			t.Fatal(err)
		}
		accBefore := account{
			Balance: stateBefore.GetBalance(addr).ToBig(),
			Nonce:   stateBefore.GetNonce(addr),
			Code:    stateBefore.GetCode(addr),
			Storage: stateBefore.GetStorageRoot(addr),
		}
		exclusions, err := api.FindCompleteExclusionList(context.Background(), initialExclusions, c.start, endBlockNum)
		if (err != nil) != c.expectErr {
			t.Fatalf("expected err %v, got %v", c.expectErr, err)
		}
		if err == nil {
			if len(exclusions.Txs) != c.expectedCompExclusions {
				t.Fatalf("expected %d txs, got %d", c.expectedCompExclusions, len(exclusions.Txs))
			}
			if exclusions.TraversedRangeEnd.BlockNumber != int64(totalBlocks) {
				t.Fatalf("expected traversed range end to be %d, got %d", totalBlocks, exclusions.TraversedRangeEnd.BlockNumber)
			}
			if exclusions.TraversedRangeStart.BlockNumber != int64(c.start) {
				t.Fatalf("expected traversed range start to be %d, got %d", c.start, exclusions.TraversedRangeStart.BlockNumber)
			}
			if exclusions.CurrentTip.BlockNumber != int64(totalBlocks) {
				t.Fatalf("expected current tip to be %d, got %d", totalBlocks, exclusions.CurrentTip.BlockNumber)
			}
		}
		stateAfter, err := backend.chain.State()
		if err != nil {
			t.Fatal(err)
		}
		accAfter := account{
			Balance: stateAfter.GetBalance(addr).ToBig(),
			Nonce:   stateAfter.GetNonce(addr),
			Code:    stateAfter.GetCode(addr),
			Storage: stateAfter.GetStorageRoot(addr),
		}

		require.Equal(t, accBefore, accAfter)
	}
}
