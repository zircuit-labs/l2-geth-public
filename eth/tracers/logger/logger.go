...
//go:generate go tool gencodec -type StructLog -field-override structLogMarshaling -out gen_structlog.go
...
type StructLog struct {
	...
	ExtraData     *types.ExtraData            `json:"extraData,omitempty"` //Zircuit-Legacy
	Err           error                       `json:"-"`
}
...
type StructLogger struct {
	...
	bytecodes map[common.Hash]CodeInfo //Zircuit-Legacy
	...
	logs       []*StructLog      // Zircuit-legacy logs
	resultSize int
	...
	statesAffected  map[common.Address]struct{} //Zircuit-Legacy
	createdAccount  *types.AccountWrapper       //Zircuit-Legacy
	callStackLogInd []int                       //Zircuit-Legacy
}
...
func (l *StructLogger) OnEnter(depth int, typ byte, from common.Address, to common.Address, input []byte, gas uint64, value *big.Int) {
	if create := (vm.OpCode(typ) == vm.CREATE || vm.OpCode(typ) == vm.CREATE2); create {	
...
func (l *StructLogger) OnOpcode(pc uint64, opcode byte, gas, cost uint64, scope tracing.OpContext, rData []byte, depth int, err error) {
	...
	log := StructLog{pc, op, gas, cost, nil, len(memory), nil, nil, nil, depth, l.env.StateDB.GetRefund(), nil, err}
...
func (l *StructLogger) GetResult() (json.RawMessage, error) {
	...
	logs := make([]json.RawMessage,len(l.logs))
	for _, log := range l.logs{
		logs = append(logs, log.toLegacyJSON())
	}
