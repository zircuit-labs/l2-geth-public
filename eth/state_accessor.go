...
// hashState opens Trie/ZKTrie state, handling multiple scenarios. On
// zkTrieSwitch-1 block migrated ZKTrie state is opened if migration is enabled
// and completed.
func (eth *Ethereum) hashState(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	var (
		...
		trieCfg  = trie.HashDefaults()
		report   = true
		origin   = block.NumberU64()
	)
	if eth.blockchain.Config().IsZKTrieSwitch(new(big.Int).Add(block.Number(), common.Big1)) {
		trieCfg.IsZktrie = true
	}
	...
	if readOnly {
		...
		h := eth.blockchain.HeaderOrWaitZKTrie(block.Header())
		statedb, err = eth.blockchain.StateAtHeader(h)
		if err == nil {
			eth.blockchain.TrieDB().Reference(h.Root, common.Hash{})
			return statedb, func() {
				eth.blockchain.TrieDB().Dereference(h.Root)
			}, nil
		}
	}
	...
	if base != nil {
		if preferDisk {
			...
			database = state.NewDatabaseWithConfig(eth.chainDb, trieCfg)
			if statedb, err = state.New(h.Root, database, nil); err == nil {
				log.Info("Found disk backend for state trie", "root", h.Root, "number", block.Number())
		...
		// TODO(rjl493456442), clean cache is disabled to prevent memory leak,
		// please re-enable it for better performance.
		triedb = trie.NewDatabase(eth.chainDb, trieCfg)
		...
		if !readOnly {
			h := eth.blockchain.HeaderOrWaitZKTrie(current.Header())
			statedb, err = state.New(h.Root, database, nil)
			if err == nil {
				return statedb, noopReleaser, nil
			}
		}
	...
	for current.NumberU64() < origin {
		...
		// zkTrieSwitch-1 has Trie root, next block must work with ZKTrie state
		h := eth.blockchain.HeaderOrWaitZKTrie(current.Header())
		if h.Root != root {
			root = h.Root
		}
...
func (eth *Ethereum) pathState(block *types.Block) (*state.StateDB, func(), error) {
	...
	statedb, err := eth.blockchain.StateAt(block.Root()) // pathdb is not supported by ZKTrie
	if err == nil {
		return statedb, noopReleaser, nil
	}
