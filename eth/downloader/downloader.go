...
import (
	...
	"github.com/zircuit-labs/l2-geth"
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.body().Transactions, result.body().Uncles).WithWithdrawals(result.Withdrawals)
	}
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	block := types.NewBlockWithHeader(result.Header).WithBody(result.body().Transactions, result.body().Uncles).WithWithdrawals(result.Withdrawals)
