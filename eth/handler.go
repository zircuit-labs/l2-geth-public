...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/core/forkid"
	...
	"github.com/zircuit-labs/l2-geth-public/eth/fetcher"
...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	if config.Sync == downloader.FullSync {
		...
		} else if !h.chain.HasStateWithNum(fullBlock.Root, fullBlock.Number.Uint64()) {
			h.snapSync.Store(true)
			log.Warn("Switch sync mode from full sync to snap sync", "reason", "head state missing")
		}
	} else {
		head := h.chain.CurrentBlock()
		if head.Number.Uint64() > 0 && h.chain.HasStateWithNum(head.Root, head.Number.Uint64()) &&
			(!config.Chain.Config().IsOptimism() || head.Number.Cmp(config.Chain.Config().BedrockBlock) != 0) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		if h.merger.TDDReached() {
			...
			// pow blocks. Spec: https://github.com/zircuit-labs/EIPs/blob/master/EIPS/eip-3675.md#halt-the-importing-of-pow-blocks
