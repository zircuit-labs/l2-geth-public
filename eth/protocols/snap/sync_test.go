...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/trie/testutil"
...
var (
	...
	poseidonCodehashes = []common.Hash{
		poseidon.CodeHash([]byte{0}),
		poseidon.CodeHash([]byte{1}),
		poseidon.CodeHash([]byte{2}),
		poseidon.CodeHash([]byte{3}),
		poseidon.CodeHash([]byte{4}),
		poseidon.CodeHash([]byte{5}),
		poseidon.CodeHash([]byte{6}),
		poseidon.CodeHash([]byte{7}),
	}
)
...
func makeAccountTrieWithStorageWithUniqueStorage(scheme string, accounts, slots int, code bool) (string, *trie.Trie, []*kv, map[common.Hash]*trie.Trie, map[common.Hash][]*kv) {
	...
	db.Update(root, types.EmptyLegacyTrieRootHash, 0, nodes, nil)
...
func makeStorageTrieWithSeed(owner common.Hash, n, seed uint64, db *trie.Database) (common.Hash, *trienode.NodeSet, []*kv) {
	trie, _ := trie.New(trie.StorageTrieID(types.EmptyLegacyTrieRootHash, owner, types.EmptyLegacyTrieRootHash), db)
	var entries []*kv
...
func makeUnevenStorageTrie(owner common.Hash, slots int, db *trie.Database) (common.Hash, *trienode.NodeSet, []*kv) {
	var (
		entries []*kv
		tr, _   = trie.New(trie.StorageTrieID(types.EmptyLegacyTrieRootHash, owner, types.EmptyLegacyTrieRootHash), db)
...
func verifyTrie(scheme string, db ethdb.KeyValueStore, root common.Hash, t *testing.T) {
	...
	for accIt.Next() {
		...
		if err := rlp.DecodeBytes(accIt.Value, &acc); err != nil {
			t.Fatalf("Invalid account encountered during snapshot creation: err %s", err)
		}
		accounts++
		if acc.Root != types.EmptyLegacyTrieRootHash {
