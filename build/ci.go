...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/crypto/signify"
	"github.com/zircuit-labs/l2-geth-public/internal/build"
...
func doInstall(cmdline []string) {
	...
	otherBuildTags := []string{"urfave_cli_no_docs"}
	// Enable linking the CKZG library since we can make it work with additional flags.
	if env.UbuntuVersion != "trusty" {
		otherBuildTags = append(otherBuildTags, "ckzg")
	}
	allBuildTags := append(otherBuildTags, *buildtags)

	// Configure the build.
	gobuild := tc.Go("build", buildFlags(env, *staticlink, allBuildTags)...)
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/zircuit-labs/l2-geth-public/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/zircuit-labs/l2-geth-public/internal/version.gitDate="+env.Date)
	}
	if env.Tag != "" {
		ld = append(ld, "-X", "github.com/zircuit-labs/l2-geth-public/params.gitTag="+env.Tag)
	}
...
func downloadSpecTestFixtures(csdb *build.ChecksumDB, cachedir string) string {
	...
	url := fmt.Sprintf("https://github.com/zircuit-labs/execution-spec-tests/releases/download/v%s/%s%s", executionSpecTestsVersion, base, ext)
