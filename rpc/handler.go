...
func (h *handler) handleCall(cp *callProc, msg *jsonrpcMessage) (answer *jsonrpcMessage) {
	if _, ok := os.LookupEnv("DD_APM_ENABLED"); ok {
		span, _ := tracer.StartSpanFromContext(cp.ctx, msg.Method)
		span.SetTag("rpc.id", string(msg.ID))
		span.SetTag("rpc.method", msg.Method)
		span.SetTag("rpc.params", string(msg.Params))
		span.SetTag("rpc.version", msg.Version)

		defer func() {
			var opts []tracer.FinishOption
			if answer != nil && answer.Error != nil {
				opts = append(opts, tracer.WithError(answer.Error))
			}
			span.Finish(opts...)
		}()
	}
