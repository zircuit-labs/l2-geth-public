...
func New(eth Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(header *types.Header) bool, cccConfig circuitcapacitychecker.Config) *Miner {
	miner := &Miner{
		mux:     mux,
		eth:     eth,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, eth, mux, isLocalBlock, true, cccConfig),
	}
	miner.wg.Add(1)
	go miner.update()
	return miner
