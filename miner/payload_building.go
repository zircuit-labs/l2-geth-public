...
func (payload *Payload) update(r *newPayloadResult, elapsed time.Duration) {
	...
	//update any deposit transactions flagged by ccc before raising an error
...
func (payload *Payload) ResolveEmpty() *engine.ExecutionPayloadEnvelope {
	...
	return engine.BlockToExecutableData(payload.empty, big.NewInt(0), nil, payload.l1Info)
}
...
func (payload *Payload) resolve(onlyFull bool) *engine.ExecutionPayloadEnvelope {
	...
	if payload.full != nil {
		return engine.BlockToExecutableData(payload.full, payload.fullFees, payload.sidecars, payload.l1Info)
	} else if !onlyFull && payload.empty != nil {
		return engine.BlockToExecutableData(payload.empty, big.NewInt(0), nil, payload.l1Info)
	} else if err := payload.err; err != nil {
		var cccErr *CCCError
		if errors.As(err, &cccErr) {
			log.Warn("Txs failed ccc while building a payload", "id", payload.id, "err", err)
			var excludedDeposits []common.Hash
			for k, _ := range payload.DepositExclusionsM.RejectedTxs {
			...
			return &engine.ExecutionPayloadEnvelope{
				RejectedPayloadM: &engine.RejectedPayloadMeta{
					TxsRejectedByCCC: excludedDeposits,
				},
				ExecutionErr: &engine.ExecutionErr{
					Err: err.Error()},
			}
		}
