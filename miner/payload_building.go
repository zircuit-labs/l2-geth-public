...
// https://github.com/zircuit-labs/execution-apis/blob/main/src/engine/cancun.md#payloadattributesv3
type BuildPayloadArgs struct {
	...
	L1Info            *types.L1Info // Zircuit addition: L1 info for the payload
	CheckTransactions bool          // Zircuit addition: whether transactions should be checked for limits
}
...
type depositExclusionsMeta struct {
	RejectedTxs             map[common.Hash]struct{}
...
func newPayload(lifeCtx context.Context, empty *types.Block, emptyRequests [][]byte, id engine.PayloadID) *Payload {
...
func (payload *Payload) update(r *newPayloadResult, elapsed time.Duration) {
	...
	// update any deposit transactions flagged by sls before raising an error
	for _, tx := range r.depositExclusions {
		payload.DepositExclusionsM.RejectedTxs[tx] = struct{}{}
	}

	var slsErr *slsCommon.WorkerError
	if errors.Is(r.err, errInterruptedUpdate) {
		log.Debug("Ignoring interrupted payload update", "id", payload.id)
		return
	} else if errors.As(r.err, &slsErr) {
		log.Warn("Block failed SLS while building a payload", "id", payload.id, "err", r.err, "poolTransactionsFlagged", slsErr.PoolTransactionsFlagged, "depositTransactionsFlagged", slsErr.DepositTransactionsFlagged)
		payload.DepositExclusionsM.PoolTransactionsFlagged = slsErr.PoolTransactionsFlagged
	}
	if r.err != nil {
...
func (payload *Payload) ResolveEmpty() *engine.ExecutionPayloadEnvelope {
	...
	return engine.BlockToExecutableData(payload.empty, big.NewInt(0), nil, payload.emptyRequests, payload.l1Info)
}
...
func (payload *Payload) resolve(onlyFull bool) *engine.ExecutionPayloadEnvelope {
	...
	if payload.full == nil && (onlyFull || payload.empty == nil) {
		...
		// If payload exec encountered an error then don't wait
		if payload.err == nil {
			payload.cond.Wait()
		}
	}
	...
	if payload.full != nil {
		return engine.BlockToExecutableData(payload.full, payload.fullFees, payload.sidecars, payload.requests, payload.l1Info)
	} else if !onlyFull && payload.empty != nil {
		return engine.BlockToExecutableData(payload.empty, big.NewInt(0), nil, payload.requests, payload.l1Info)
	} else if err := payload.err; err != nil {
		var excludedDeposits []common.Hash
		for k := range payload.DepositExclusionsM.RejectedTxs {
			excludedDeposits = append(excludedDeposits, k)
		}

		var slsErr *slsCommon.WorkerError
		if errors.As(err, &slsErr) {
			if slsErr.DepositTransactionsFlagged {
				log.Warn("Block failed SLS while building a payload", "id", payload.id, "err", err, "depositTransactionsFlagged", slsErr.DepositTransactionsFlagged)
			}
			if slsErr.PoolTransactionsFlagged {
				log.Warn("Block failed SLS while building a payload", "id", payload.id, "err", err, "poolTransactionsFlagged", slsErr.PoolTransactionsFlagged)
			}
			return createRejectedPayloadEnvelopeSLSError(excludedDeposits, slsErr)
		}

		log.Error("Error building a payload", "id", payload.id, "err", err)
	}
	log.Info("returning nil in resolve")
	return nil
}

// createRejectedPayloadEnvelopeSLSError creates an ExecutionPayloadEnvelope for a block that failed SLS validation.
// It includes metadata about transactions flagged and error details from the SLSError.
func createRejectedPayloadEnvelopeSLSError(excludedDeposits []common.Hash, slsErr *slsCommon.WorkerError) *engine.ExecutionPayloadEnvelope {
	if slsErr == nil {
		return nil
	}

	txsFlagged := false
	if slsErr.PoolTransactionsFlagged || slsErr.DepositTransactionsFlagged {
		txsFlagged = true
	}

	return &engine.ExecutionPayloadEnvelope{
		RejectedPayloadM: &engine.RejectedPayloadMeta{
			TxsRejectedBySLS: excludedDeposits,
			BlockFailedSLS:   txsFlagged,
		},
		ExecutionErr: &engine.ExecutionErr{Err: slsErr.Error()},
	}
}
...
func (miner *Miner) buildPayload(args *BuildPayloadArgs) (*Payload, error) {
	if args.NoTxPool { // don't start the background payload updating job if there is no tx pool to pull from
		...
		emptyParams := &generateParams{
			...
			checkTransactions: args.CheckTransactions,
		}
		empty := miner.generateWork(emptyParams)
		if empty.err != nil {
			return nil, empty.err
		}
		payload := newPayload(miner.lifeCtx, empty.block, empty.requests, args.Id())
