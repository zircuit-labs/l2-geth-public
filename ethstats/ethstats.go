...
import (
	...
	ethproto "github.com/zircuit-labs/l2-geth/eth/protocols/eth"
...
// fork data that is passed to ethstats
type fork struct {
...
func (s *Service) login(conn *connWrapper) error {
	...
	if miningBackend, ok := s.backend.(miningNodeBackend); ok {
		chainConfig = miningBackend.Miner().GetChainConfig()
	}
	...
	if chainConfig != nil {
		auth.Info.Forks = filterForks(chainConfig)
	}
...
// filterForks extracts ChainConfig fields with *Time
// returns last 3 forks which are active
// active if forkTime == 0 or forkTime > time.Now()
func filterForks(chainConfig *params.ChainConfig) []fork {
	out := []fork{}

	v := reflect.ValueOf(chainConfig).Elem()
	typ := v.Type()
	...
	// loop over ChainConfig struct fields
	for i := range typ.NumField() {
		...
		if strings.HasSuffix(field, "Time") {
			...
			if fieldValue.Kind() == reflect.Ptr && !fieldValue.IsNil() {
				...
				timeValue := fieldValue.Elem().Uint()
				forkTime := time.Unix(int64(timeValue), 0)

				// active if time equals 0 or is before now
				if timeValue == 0 || forkTime.Before(time.Now()) {

					forkName := strings.TrimSuffix(field, "Time")
					fork := fork{Name: forkName, Time: timeValue}
					out = append(out, fork)
				}
			}
		}
	}
	// keep last three forks
	l := len(out)
	if l > 3 {
		out = out[l-3:]
	}
