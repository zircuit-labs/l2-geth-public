...
// ZKTrie state is only checked when ZKTrie is globally enabled via the flag.
func (bc *BlockChain) HasState(hash common.Hash) bool {
	_, err := bc.stateCache.OpenTrie(hash)
	return err == nil
}

// HasStateWithNum is like HasState, but ZK state is checked if ZKTrie block
// number was passed. This method is preferred over HasState when the block
// number is available and ZKTrie logic should be accessible with migration
// enabled.
func (bc *BlockChain) HasStateWithNum(root common.Hash, number uint64) bool {
	if !bc.isZkTrieSwitch(number) || bc.zkTrieCache == nil {
		return bc.HasState(root)
	}

	db := state.NewDatabaseWithNodeDB(bc.db, bc.zkTrieCache)
	_, err := db.OpenTrie(root)
	return err == nil
}

// HasBlockAndState checks if a block and associated state trie is fully present
// in the database or not, caching it if present. ZKTrie state can be also
// checked if number >= zkTrieSwitch.
func (bc *BlockChain) HasBlockAndState(hash common.Hash, number uint64) bool {
	...
	return bc.HasStateWithNum(block.Root(), number)
}
...
func (bc *BlockChain) State() (*state.StateDB, error) {
	return bc.StateAtHeader(bc.CurrentBlock())
}

// StateAt returns a new mutable state based on a particular point in time.
// ZKTrie state is only returned when ZKTrie is globally enabled via the flag.
func (bc *BlockChain) StateAt(root common.Hash) (*state.StateDB, error) {
	return state.New(root, bc.stateCache, bc.snaps)
}

// StateAtHeader is like StateAt, but returns ZKTrie state for blocks >=
// zkTrieSwitch, waiting for migration to finish on need. The header is required
// only for block number and root.
//
// This method is preferred over StateAt when the header is available and ZKTrie
// logic with migration enabled should be accessible.
func (bc *BlockChain) StateAtHeader(header *types.Header) (*state.StateDB, error) {
	if !bc.isZkTrieSwitch(header.Number.Uint64()) || bc.zkTrieCache == nil {
		db, err := bc.StateAt(header.Root)
		if err != nil {
			return nil, fmt.Errorf("get trie state at header=%d root=%x: %v", header.Number, header.Root, err)
		}
		return db, nil
	}

	db, err := state.New(header.Root, state.NewDatabaseWithNodeDB(bc.db, bc.zkTrieCache), nil)
	if err != nil {
		return nil, fmt.Errorf("get zktrie state at header=%d root=%x: %v", header.Number, header.Root, err)
	}
	return db, nil
}

// HeaderOrWaitZKTrie returns passed header or dummy for StateAtHeader depending
// on block number. The header at number zkTrieSwitch-1 is special: it has Trie
// state root, but we should open ZKTrie on the root where migration has finished
// instead. For the next blocks we will have ZKTrie root set in the header
// already.
//
// Use this function only when you are sure you need ZKTrie state at zkTrieSwitch-1.
func (bc *BlockChain) HeaderOrWaitZKTrie(header *types.Header) *types.Header {
	var (
		next = header.Number.Uint64() + 1
		root = header.Root
		zk   = bc.Config().ZKTrieSwitchBlock
	)
	if bc.isZkTrieSwitch(next) && zk != nil && zk.Uint64() == next {
		if bc.zkTrieCache == nil {
			log.Warn("Waiting for ZKTrie migration to finish", "block", next-1)
			<-bc.zkMigrator.done
		}
		root = bc.zkMigrator.parentRoot
		log.Info("Using final migration root before ZKTrie block", "block", next-1, "root", root.Hex())
		// overriding number to meet isZkTrieSwitch condition
		return &types.Header{Root: root, Number: zk}
	}
	return header
}

func (bc *BlockChain) isZkTrieSwitch(block uint64) bool {
	return bc.Config().IsZKTrieSwitch(new(big.Int).SetUint64(block)) && bc.zkMigrator != nil
}
