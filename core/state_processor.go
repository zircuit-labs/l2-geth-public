...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (*ProcessResult, error) {
	...
	if p.config.IsTenrec(block.Time()) {
	...
	isTenrec := p.config.IsTenrec(block.Time())
	// Read requests if Prague is enabled.
	var requests [][]byte
	if p.config.IsPrague(block.Number(), block.Time()) && !isTenrec {
		requests = [][]byte{}
		// EIP-6110
...
func ApplyTransactionWithEVM(msg *Message, gp *GasPool, statedb *state.StateDB, blockNumber *big.Int, blockHash common.Hash, blockTime uint64, tx *types.Transaction, usedGas *uint64, evm *vm.EVM) (receipt *types.Receipt, err error) {
	...
	// If a limiter is configured (miner path), turn EVMLimitError into a hard error
	if err == nil && result != nil && evm.ExecutionLimiter() != nil {
		if vm.IsEVMLimitError(result.Err) {
			// Turn this into a hard error so worker.generateWork can see it
