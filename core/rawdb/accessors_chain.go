...
func ReadReceipts(db ethdb.Reader, hash common.Hash, number uint64, time uint64, config *params.ChainConfig) types.Receipts {
	...
	if err := receipts.DeriveFields(config, hash, number, time, baseFee, blobGasPrice, body.Transactions,l1Info); err != nil {
...
func ReadBadBlock(db ethdb.Reader, hash common.Hash) *types.Block {
	...
	for _, bad := range badBlocks {
		if bad.Header.Hash() == hash {
			...
			if bad.Body != nil {
				block = block.WithBody(bad.Body.Transactions, bad.Body.Uncles).WithWithdrawals(bad.Body.Withdrawals)
			}
			return block
		}
	}
