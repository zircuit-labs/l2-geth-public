...
func testHeaderVerification(t *testing.T, scheme string) {
	...
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), gspec, ethash.NewFaker(), options)
	defer chain.Stop()
...
func TestTransactionCountLimit(t *testing.T) {
	...
	*config.Scroll.MaxTxPerBlock = 1
	...
	var (
		...
		gspec         = &Genesis{Config: &config, Alloc: types.GenesisAlloc{address: {Balance: funds}}}
		db, blocks, _ = GenerateChainWithGenesis(gspec, ethash.NewFaker(), 8, nil)
	)
	headers := make([]*types.Header, len(blocks))
	for i, block := range blocks {
		headers[i] = block.Header()
	...
	if _, err := chain.InsertChain(block1); err != nil {
	...
	if _, err := chain.InsertChain(block2); err != nil {
		t.Fatalf("failed to insert chain: %v", err)
	}
	...
	_, err = chain.InsertChain(block3)
...
// TestTransactionCountLimitPostAlfie tests that the chain no longer rejects blocks based on the
// Scroll.MaxTxPerBlock config
func TestTransactionCountLimitPostAlfie(t *testing.T) {
	// Create config that allows at most 1 transaction per block
	config := *params.TestChainConfig
	config.Scroll.MaxTxPerBlock = new(int)
	*config.Scroll.MaxTxPerBlock = 1
	alfieTime := uint64(0)
	config.AlfieTime = &alfieTime
	...
	if _, err := chain.InsertChain(block3); err != nil {
		t.Fatalf("failed to insert chain even though tx limit should no longer be active: %v", err)
	}
}
