...
func New(gasTip uint64, chain BlockChain, subpools []SubPool) (*TxPool, error) {
	...
	for i, subpool := range subpools {
		if err := subpool.Init(ctx, gasTip, head, reserver.NewHandle(i)); err != nil {
			for j := i - 1; j >= 0; j-- {
				subpools[j].Close()
			}
			return nil, err
...
func (p *TxPool) RemoveTx(hash common.Hash, outofbound bool, unreserve bool) {
	for _, subpool := range p.subpools {
		subpool.RemoveTx(hash, outofbound, unreserve)
	}
}
