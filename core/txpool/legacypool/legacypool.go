...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/common/prque"
...
type (
	...
	quarantiner interface {
		SendToQuarantine(ctx context.Context, tx *types.Transaction, detector, reason string, loss uint64) error
	...
	txprocessor interface {
		ProcessTransactionsWithTimeout(ctx context.Context, addr common.Address, transactions types.Transactions) types.Transactions
		CollectSendBackToPool() types.Transactions
		CollectPromotablesByAddrss(addr common.Address) types.Transactions
...
func New(config Config, chain BlockChain, slsConfig sls.Config) *LegacyPool {
...
func (pool *LegacyPool) Init(gasTip *big.Int, head *types.Header, reserve txpool.AddressReserver) error {
	...
	// fully synced).
	statedb, err := pool.chain.StateAtHeader(head)
	if err != nil {
		empty := types.EmptyLegacyTrieRootHash
		if params.ZKTrieEnabled.Load() {
			// if globally enabled, StateAt returns ZKTrie
			empty = types.EmptyZkTrieRootHash
		}
	...
	if pool.slsConfig.Enabled {

		slsDatabase, err := storage.NewStorage(context.TODO(), storage.Config{DSN: pool.slsConfig.DSN})
		if err != nil {
		...
		pool.detectorManager = sls.NewManager(detectors, trustVerifiers, slsDatabase)

		pool.quarantiner = sls.NewQuarantiner(slsDatabase, time.Duration(pool.slsConfig.QuarantineDuration), pool.signer)

		pool.executor = NewExecutor(pool.quarantiner, pool.signer, pool, time.Duration(pool.slsConfig.ExecutorInterval))
	}
	...
	if pool.slsConfig.Enabled {
		pool.wg.Add(1)
		go pool.executor.Loop(context.TODO())
	}
...
func (pool *LegacyPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	for _, addr := range accounts {
		...
		balance := pool.currentState.GetBalance(addr).ToBig()
		...
		promotables := pool.txprocessor.ProcessTransactionsWithTimeout(ctx, addr, readies)
		...
		// Send back to the queue if any transactions had a failure.
		for _, tx := range sendBackToPool {
			if _, err := pool.add(tx, false); err != nil {
				logger.With("tx_hash", tx.Hash().String(), "err", err).Warn("Can't add transaction back to the Pool")
			}
		}
