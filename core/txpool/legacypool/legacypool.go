...
import (
	...
	"github.com/zircuit-labs/l2-geth/common/prque"
	...
	"github.com/zircuit-labs/l2-geth/core/sls-public/processor"
...
//go:generate go tool mockgen -source legacypool.go -destination mock_legacypool.go -package legacypool
...
const (
	...
	// defaultPondPoolMaxConcurrency is the default maximum concurrency for the pond pool
	// used when PondPoolMaxConcurrency is not set or is invalid (zero or negative).
	defaultPondPoolMaxConcurrency = 100
)
...
var (
	...
	// ErrAlreadyScannedBySLS is returned if a tx cannot be put into the future queue
	// because it has already been scanned by SLS
	ErrAlreadyScannedBySLS = errors.New("tx has already been scanned by SLS")

	// ErrSignatureAddress is returned if a tx signature does not contain an address
	ErrSignatureAddress = errors.New("no address in tx signature")
)
...
type LegacyPool struct {
	...
	getRefreshableConfigs sls.RefreshableGetter // Returns the latest SLS refreshable configs from the live-reloading reader on each call

	slsConfig       sls.Config // Static configs which loaded once at startup
	detectorManager slsCommon.DetectorManager
	quarantiner     slsCommon.Quarantiner
	executor        Executor
	txprocessor     slsCommon.TXProcessor
	slsCloser       func()
}

func (pool *LegacyPool) SLSConfig() sls.Config {
	return pool.slsConfig
}

func (pool *LegacyPool) SetSLSConfig(cfg sls.Config) {
	pool.slsConfig = cfg
}

func (pool *LegacyPool) SetSLSRefreshables(fn sls.RefreshableGetter) {
	pool.getRefreshableConfigs = fn
}

func (pool *LegacyPool) GetSLSRefreshables() sls.RefreshableConfigs {
	return pool.getRefreshableConfigs()
}

func (pool *LegacyPool) DetectorManager() slsCommon.DetectorManager {
	return pool.detectorManager
}

func (pool *LegacyPool) SetDetectorManager(dm slsCommon.DetectorManager) {
	pool.detectorManager = dm
}

func (pool *LegacyPool) Quarantiner() slsCommon.Quarantiner {
	return pool.quarantiner
}

func (pool *LegacyPool) SetQuarantiner(q slsCommon.Quarantiner) {
	pool.quarantiner = q
}

func (pool *LegacyPool) TxFeed() slsCommon.EventSender {
	return &pool.txFeed
}

func (pool *LegacyPool) GetSigner() types.Signer {
	return pool.signer
}

func (pool *LegacyPool) SetExecutor(e Executor) {
	pool.executor = e
}
...
func New(config Config, chain BlockChain, slsConfig sls.Config, getRefreshableConfigs sls.RefreshableGetter) *LegacyPool {
	...
	pool := &LegacyPool{
		...
		slsConfig:             slsConfig,
		getRefreshableConfigs: getRefreshableConfigs,
	}
...
// If this pool has SLS enabled, set the detector manager it should use.
func (pool *LegacyPool) AddDetectorManager(detectorManager slsCommon.DetectorManager) error {
	if !pool.slsConfig.IsEnabled() {
		return errors.New("attempt to add detector manager when SLS is not enabled")
	}

	pool.detectorManager = detectorManager
	return nil
}
...
func (pool *LegacyPool) WithDetectors(ctx context.Context) error {
	if !pool.slsConfig.IsEnabled() {
		return errors.New("SLS disabled on pool")
	}

	if pool.detectorManager == nil {
		return errors.New("SLS enabled on pool but no detectors present")
	}

	slsDatabase, err := storage.NewStorage(ctx, pool.slsConfig)
	if err != nil {
		log.Warn("Can't connect to SLS storage", "err", err)
		return err
	}

	pool.quarantiner = sls.NewQuarantiner(ctx, slsDatabase, pool.slsConfig, pool.getRefreshableConfigs, pool.signer)

	pool.executor = sls.NewExecutor(pool.quarantiner, pool.signer, pool)
...
func (pool *LegacyPool) Init(ctx context.Context, gasTip uint64, head *types.Header, reserver txpool.Reserver) error {
	...
	if err != nil {
		return fmt.Errorf("couldn't retrieve state at head or empty: %w", err)
	}
	...
	if pool.slsConfig.IsEnabled() {
		maxConcurrency := pool.SLSConfig().PondPoolMaxConcurrency
		if maxConcurrency <= 0 {
			maxConcurrency = defaultPondPoolMaxConcurrency
		}
		pondPool := pond.NewPool(maxConcurrency)
		pool.txprocessor = processor.New(
			pool.detectorManager,
			pool.quarantiner,
			pondPool,
			pool.getRefreshableConfigs,
		)
	}
	...
	if pool.slsConfig.IsEnabled() {
		pool.wg.Add(1)
		go pool.executor.Loop(ctx, &pool.wg, &pool.mu)
	}
...
func (pool *LegacyPool) Close() error {
	...
	if pool.slsConfig.IsEnabled() {
		pool.executor.Stop()
		if pool.txprocessor != nil {
			pool.txprocessor.Stop()
		}
	}

	pool.wg.Wait()

	if pool.slsCloser != nil {
		pool.slsCloser()
		pool.slsCloser = nil
	}
...
func (pool *LegacyPool) validateTx(tx *types.Transaction, _ bool) error {
...
func (pool *LegacyPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	...
	if list := pool.pending[from]; list != nil && list.Contains(tx.Nonce()) {
		shouldReplace, oldHash := list.ShouldReplace(tx, pool.config.PriceBump)
		if !shouldReplace {
			pendingDiscardMeter.Mark(1)
			return false, txpool.ErrReplaceUnderpriced
		}

		// Demote the current one, SLS will handle actual promotion
		if oldHash != nil {
			pool.removeTx(*oldHash, true, false) // Keep reservation
		}
	}
...
func (pool *LegacyPool) enqueueTx(hash common.Hash, tx *types.Transaction, local bool, addAll bool) (bool, error) {
	...
	if err != nil {
		return false, ErrSignatureAddress
	}

	// Check if tx has already been scanned by SLS
	if ok := pool.enqueueConflict(from, tx); ok {
		return false, ErrAlreadyScannedBySLS
	}
...
// enqueueConflict checks if incoming txs are duplicates of txs being processed already
func (pool *LegacyPool) enqueueConflict(from common.Address, tx *types.Transaction) bool {
	// checking queue of promoted txs
	if list, ok := pool.pending[from]; ok {
		if txSameNonce := list.Get(tx.Nonce()); txSameNonce != nil {
			if txSameNonce.Hash().Cmp(tx.Hash()) == 0 {
				return true
			}
		}
	}
	// checking queue of to be promoted txs
	if pool.txprocessor != nil {
		for _, awaitedTx := range pool.txprocessor.Awaited(from) {
			if awaitedTx.Hash().Cmp(tx.Hash()) == 0 {
				return true
			}
		}
	}
	// checking against the latest cached quarantine txs
	if pool.quarantiner != nil {
		if pool.quarantiner.WasRecentlyQuarantined(tx.Hash().Hex()) {
			return true
		}
	}
	return false
...
func (pool *LegacyPool) RemoveTx(hash common.Hash, outofbound bool, unreserve bool) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
...
func (pool *LegacyPool) scheduleReorgLoop() {
	...
	var (
		...
		queuedEvents  = make(map[common.Address]*types.SortedMap)
	)
	for {
		...
		case tx := <-pool.queueTxEventCh:
			...
			if _, ok := queuedEvents[addr]; !ok {
				queuedEvents[addr] = types.NewSortedMap()
			}
...
func (pool *LegacyPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*types.SortedMap) {
	...
	for _, tx := range promoted {
		...
		if _, ok := events[addr]; !ok {
			events[addr] = types.NewSortedMap()
		}
		events[addr].Put(tx)
	}
	if len(events) > 0 {
...
func (pool *LegacyPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	logger := slslog.NewWith("legacy_pool", true)

	// Load the enable flag as a refreshable config
	slsEnabled := pool.getRefreshableConfigs().Enabled

	if slsEnabled {
		// Promote all processed transactions from the previous cycle first
		promotables := pool.txprocessor.CollectAllPromotables()
		for _, tx := range promotables {
			addr, err := types.Sender(pool.signer, tx)
			if err != nil {
				logger.With("tx_hash", tx.Hash().String(), "err", err).Error("Error retrieving transaction sender")
				continue
			}
			if pool.PromoteTx(addr, tx.Hash(), tx) {
				promoted = append(promoted, tx)
				logger.With("tx_hash", tx.Hash().String()).Debug("Transaction promoted")
			}

			pool.cleanupAddressQueueIfEmpty(addr)
		}

		// Try to clean up addresses that had transactions sent to quarantine
		for _, addr := range pool.txprocessor.CollectQuarantinedAddresses() {
			pool.cleanupAddressQueueIfEmpty(addr)
		}
	}
	...
	for _, addr := range accounts {
		...
		// The returned transactions will be removed from the list.
		...
		if slsEnabled {
			// Send a list of transactions to process.
			// Collects and returns only the transactions that have been processed within the specified timeout period.
			// The slsworkers will continue processing any in-progress transactions and will process the remaining transactions in the background.
			// Transactions that miss this promotion cycle will be included in the next cycle once processing is complete.
			promotables = pool.txprocessor.ProcessTransactions(ctx, addr, readies)
			sendBackToPool = pool.txprocessor.CollectSendBackToPool()

			logger.With("promotables", len(promotables), "sendBackToPool", len(sendBackToPool)).Debug("SLS has verified transactions for this promotion cycle")
		}
		...
		if slsEnabled && len(sendBackToPool) > 0 {
			// Asynchronously send back to the queue if any transactions had a failure.
			pool.wg.Add(1)
			go func() {
				defer pool.wg.Done()
				pool.reAddToQueue(sendBackToPool)
			}()
		}

		logger.With("count", len(promoted)).Trace("Promoted queued transactions")
...
// defaultBatchSize is the default number of transactions processed in a batch. // It can be overridden by setting ReAddBatchSize in slsConfig.
const defaultBatchSize = 100

// reAddToQueue batches transactions and re-adds them to the pool in the background.
// It uses the pool.Add method for batching and waits between batches.
func (pool *LegacyPool) reAddToQueue(txs types.Transactions) {
	batchSize := defaultBatchSize
	if reAddBatchSize := sls.GetReAddBatchSize(pool.getRefreshableConfigs); reAddBatchSize > 0 {
		batchSize = reAddBatchSize
	}

	logger := slslog.NewWith("module", "legacypool")
	failedCount := 0

	for i := 0; i < len(txs); i += batchSize {
		end := min(i+batchSize, len(txs))
		batch := txs[i:end]
		// Remove transactions from the txpool before re-adding them back. This prevents
		// `ErrAlreadyKnown` errors that can occur when SLS processing fails and transactions
		// are re-added to the pool while still present in the 'all' lookup store.
		for _, tx := range batch {
			pool.RemoveTx(tx.Hash(), true, true)
		}
		errs := pool.Add(batch, false, false)
		if len(errs) > 0 {
			logger.With("errs", errors.Join(errs...)).Warn("Failed to re-add transaction to the pool")
		}
		failedCount += len(errs)
	}

	if failedCount > 0 {
		logger.With("failed_count", failedCount, "total_count", len(txs)).Warn("Some transactions failed to re-add to pool")
	}
}

// cleanupAddressQueueIfEmpty removes a queued transaction list and associated metadata for the given address.
// If the address has no dependencies in pending, queue, or SLS, it releases resources reserved for the address.
func (pool *LegacyPool) cleanupAddressQueueIfEmpty(addr common.Address) {
	logger := slslog.NewWith("addr", addr)

	logger.Debug("Starting cleanup for address queue")

	if list := pool.queue[addr]; list == nil || !list.Empty() {
		logger.Debug("Address queue is not empty or doesnt exist, skipping cleanup")
		return
	}

	logger.Debug("Removing address from queue")
	delete(pool.queue, addr)
	delete(pool.beats, addr)

	if _, ok := pool.pending[addr]; ok {
		logger.Debug("Address found in pending transactions, skipping resource release")
		return
	}

	if pool.getRefreshableConfigs().Enabled && pool.txprocessor.IsAddressWaiting(addr) {
		logger.Debug("Address is waiting in txprocessor, skipping resource release")
		return
	}

	if err := pool.reserver.Release(addr); err != nil {
		logger.Warn("Failed to release address reservation", "err", err)
		return
	}

	logger.Debug("Address cleanup completed successfully")
}
...
func (pool *LegacyPool) truncateQueue() {
	...
	globalQueueLimit := pool.config.GlobalQueue
	thresholds := []uint64{90, 80, 70}

	// Check against thresholds and log warnings
	for _, threshold := range thresholds {
		if queued > globalQueueLimit*threshold/100 {
			log.Warn("Transaction queue capacity warning", "percentage", threshold)
			break
		}
	}

	// Log the current capacity
	currentCapacity := (queued * 100) / globalQueueLimit
	log.Info("Transaction queue current capacity", "percentage", currentCapacity)

	if queued <= globalQueueLimit {
		return
	}
	...
	log.Info("Dropped transactions due to queue limit", "totalDropped", totalDropped)
}
