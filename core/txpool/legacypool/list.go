...
type list struct {
	...
	txs    *types.SortedMap // Heap indexed sorted hash map of the transactions
...
// ShouldReplace determines if a new transaction should replace an existing one based on gas prices and a price bump threshold.
func (l *list) ShouldReplace(tx *types.Transaction, priceBump uint64) (bool, *common.Hash) {
	old := l.txs.Get(tx.Nonce())
	if old == nil {
		return true, nil // No existing tx to replace — allow
	}
...
// Get returns the transaction at the nonce or nil
...
func (l *list) Add(tx *types.Transaction, priceBump uint64) (bool, *types.Transaction) {
	if shouldReplace, _ := l.ShouldReplace(tx, priceBump); !shouldReplace {
		return false, nil
	}
...
func (l *list) Filter(costLimit *uint256.Int, gasLimit uint64) (types.Transactions, types.Transactions) {
	...
	removed := l.txs.Filter(func(tx *types.Transaction) bool {
		...
		// since the transactions are already part of the pool,
		// computing the cost again should never overflow
		if of {
	...
	if l.strict {
		...
		invalids = l.txs.FilterNoReheap(func(tx *types.Transaction) bool { return tx.Nonce() > lowest })
	}
	// Reset total cost
...
func (l *list) subTotalCost(txs []*types.Transaction) {
	for _, tx := range txs {
		...
		cost := l.txCosts[h]
		if cost == nil {
			panic("cannot compute on nil value")
		}
		_, underflow := l.totalcost.SubOverflow(l.totalcost, cost)
		// since we are subtracting a subset of transactions from the
		// total sum, this should never underflow
		if underflow {
