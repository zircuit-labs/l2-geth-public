...
func (bc *testBlockChain) StateAtHeader(header *types.Header) (*state.StateDB, error) {
	return bc.statedb, nil
}
...
func setupPoolWithConfig(config *params.ChainConfig) (*LegacyPool, *ecdsa.PrivateKey) {
	statedb, _ := state.New(types.EmptyLegacyTrieRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	blockchain := newTestBlockChain(config, 10000000, statedb, new(event.Feed))

	key, _ := crypto.GenerateKey()
	pool := New(testTxPoolConfig, blockchain, sls.DisabledConfig)
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = state.New(types.EmptyLegacyTrieRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
...
type LabeledTransactions []LabeledTransaction

type AddressTxLabelMap map[common.Address]map[common.Hash]string

/*
TestSLS evaluates the SLS feature in the LegacyPool over multiple promotion cycles to ensure correct transaction handling.

1. **Setup:** Initializes the transaction pool with addresses, balances, and necessary mocks to simulate conditions.

2. **Transaction Enqueueing:**
  - Enqueues a series of transactions across three addresses, distributed over three promotion cycles.
  - Includes a mix of regular transactions, delayed (slow) transactions, and those designated for quarantine or expected errors.

3. **Promotion Cycles:** Executes three promotion cycles, validating:
  - The number of transactions in the pending pool matches expectations, including the carryover of slow transactions from previous cycles.
  - Transactions are correctly present in the pool for each address.
  - Quarantined transactions are appropriately excluded from the pending queue.
  - Errored transactions are re-queued correctly.

4. **Assertions:** Verifies that promotion cycles either respect configured timeouts or complete within the expected time, and that transactions are processed, quarantined, or re-added accurately.
*/
func TestSLS(t *testing.T) {
	t.Parallel()

	pool, addr1Key := setupPool()
	defer pool.Close()

	addr1, _ := deriveSender(transaction(0, 100, addr1Key))
	addr2Key, _ := crypto.GenerateKey()
	addr2, _ := deriveSender(transaction(0, 100, addr2Key))
	addr3Key, _ := crypto.GenerateKey()
	addr3, _ := deriveSender(transaction(0, 100, addr3Key))

	// Test data setup
	addr1Txs := LabeledTransactions{
		// Enqueue in first promotion
		{transaction(0, 100, addr1Key), "addr1Tx1"},
		{transaction(1, 100, addr1Key), "addr1Tx2"},
		{transaction(2, 100, addr1Key), "addr1SlowTx3"},
		{transaction(3, 100, addr1Key), "addr1QuarantineTx4"},
		// Enqueue in second promotion
		{transaction(3, 100, addr1Key), "addr1Tx5"},
		{transaction(4, 100, addr1Key), "addr1Tx6"},
		// Enqueue in third promotion
		{transaction(5, 100, addr1Key), "addr1Tx7"},
		{transaction(6, 100, addr1Key), "addr1Tx8"},
	}

	addr2Txs := LabeledTransactions{
		// Enqueue in first promotion
		{transaction(0, 100, addr2Key), "addr2Tx1"},
		{transaction(1, 100, addr2Key), "addr2ErrorTx2"},
		// Enqueue in second promotion
		{transaction(1, 100, addr2Key), "addr2Tx3"},
		// Enqueue in third promotion
		{transaction(2, 100, addr2Key), "addr2QuarantineTx4"},
	}

	addr3Txs := LabeledTransactions{
		// Enqueue in second promotion
		{transaction(0, 100, addr3Key), "addr3Tx1"},
		{transaction(1, 100, addr3Key), "addr3SlowTx2"},
		// Enqueue in third promotion
		{transaction(2, 100, addr3Key), "addr3Tx3"},
		{transaction(3, 100, addr3Key), "addr3ErrorTx4"},
	}

	testAddBalance(pool, addr1, big.NewInt(1000))
	testAddBalance(pool, addr2, big.NewInt(1000))
	testAddBalance(pool, addr3, big.NewInt(1000))
	pool.reset(nil, nil)
	...
	slowTxs := types.Transactions{addr1Txs[2].tx, addr3Txs[1].tx}
	quarantineTxs := types.Transactions{addr1Txs[3].tx, addr2Txs[3].tx}
	errorTxs := types.Transactions{addr2Txs[1].tx, addr3Txs[3].tx}
	setupMocks(slowTxProcessTime, mockManager, mockQuarantiner, slowTxs, quarantineTxs, errorTxs)

	// Enable SLS and setup related mocks
	slsPromotionTimeout := 1 * time.Second
	pool.slsConfig.Enabled = true
	pool.slsConfig.MaxWorkersPerCycle = 3
	pool.slsConfig.MaxConcurrencyPool = 5
	pool.slsConfig.PromotablesCollectionTimeout = duration.Duration(slsPromotionTimeout)
	...
	pool.txprocessor = NewSLSTxProcessor(pool)

	// Queue transactions for first promotion
	enqueueTransactions(pool, addr1Txs[:4])
	enqueueTransactions(pool, addr2Txs[:2])

	accounts := []common.Address{addr1, addr2}
	expectedLengths := map[common.Address]int{
		addr1: 2, addr2: 1,
	}
	expectedTxs := AddressTxLabelMap{
		addr1: {addr1Txs[0].tx.Hash(): addr1Txs[0].label, addr1Txs[1].tx.Hash(): addr1Txs[1].label},
		addr2: {addr2Txs[0].tx.Hash(): addr2Txs[0].label},
	}
	expectedErrorTxs := []common.Hash{addr2Txs[1].tx.Hash()}
	shouldWaitForTimeout := true

	// First promotion cycle
	testPromotionCycle(t, pool, 1, slsPromotionTimeout, accounts, expectedLengths, expectedTxs, expectedErrorTxs, shouldWaitForTimeout)

	// Wait for the slow transactions to be finished
	time.Sleep(2 * time.Second)

	// Enqueue transactions for second promotion
	enqueueTransactions(pool, addr1Txs[4:6])
	enqueueTransactions(pool, addr2Txs[2:3])
	enqueueTransactions(pool, addr3Txs[:2])

	accounts = []common.Address{addr1, addr2, addr3}
	expectedLengths = map[common.Address]int{
		addr1: 5, addr2: 2, addr3: 1,
	}
	expectedTxs = AddressTxLabelMap{
		addr1: {
			addr1Txs[0].tx.Hash(): addr1Txs[0].label,
			addr1Txs[1].tx.Hash(): addr1Txs[1].label,
			addr1Txs[2].tx.Hash(): addr1Txs[2].label,
			addr1Txs[4].tx.Hash(): addr1Txs[4].label,
			addr1Txs[5].tx.Hash(): addr1Txs[5].label,
		},
		addr2: {
			addr2Txs[0].tx.Hash(): addr2Txs[0].label,
			addr2Txs[2].tx.Hash(): addr2Txs[2].label,
		},
		addr3: {addr3Txs[0].tx.Hash(): addr3Txs[0].label},
	}
	expectedErrorTxs = []common.Hash{}
	shouldWaitForTimeout = true

	// Second promotion
	testPromotionCycle(t, pool, 2, slsPromotionTimeout, accounts, expectedLengths, expectedTxs, expectedErrorTxs, shouldWaitForTimeout)

	// Wait for the slow transactions to be finished
	time.Sleep(2 * time.Second)

	// Enqueue transactions for third promotion
	enqueueTransactions(pool, addr1Txs[6:])
	enqueueTransactions(pool, addr2Txs[3:])
	enqueueTransactions(pool, addr3Txs[2:])

	accounts = []common.Address{addr1, addr2, addr3}
	expectedLengths = map[common.Address]int{
		addr1: 7, addr2: 2, addr3: 3,
	}
	expectedTxs = AddressTxLabelMap{
		addr1: {
			addr1Txs[0].tx.Hash(): addr1Txs[0].label,
			...
			addr1Txs[4].tx.Hash(): addr1Txs[4].label,
			addr1Txs[5].tx.Hash(): addr1Txs[5].label,
			addr1Txs[6].tx.Hash(): addr1Txs[6].label,
			addr1Txs[7].tx.Hash(): addr1Txs[7].label,
		},
		addr2: {
			addr2Txs[0].tx.Hash(): addr2Txs[0].label,
			addr2Txs[2].tx.Hash(): addr2Txs[2].label,
		...
		addr3: {
			addr3Txs[0].tx.Hash(): addr3Txs[0].label,
			addr3Txs[1].tx.Hash(): addr3Txs[1].label,
			addr3Txs[2].tx.Hash(): addr3Txs[2].label,
		},
	}
	expectedErrorTxs = []common.Hash{addr3Txs[3].tx.Hash()}
	shouldWaitForTimeout = false

	// Third promotion
	testPromotionCycle(t, pool, 3, slsPromotionTimeout, accounts, expectedLengths, expectedTxs, expectedErrorTxs, shouldWaitForTimeout)
}

func TestSLSWorkerLimit(t *testing.T) {
	t.Parallel()
	...
	addr1Txs := LabeledTransactions{
		// Enqueue in first promotion
		{transaction(0, 100, addr1Key), "addr1SlowTx1"},
		{transaction(1, 100, addr1Key), "addr1SlowTx2"},
		{transaction(2, 100, addr1Key), "addr1SlowTx3"},
		{transaction(3, 100, addr1Key), "addr1SlowTx4"},
	}

	addr2Txs := LabeledTransactions{
		// Enqueue in first promotion
		{transaction(0, 100, addr2Key), "addr2SlowTx1"},
		{transaction(1, 100, addr2Key), "addr2SlowTx2"},
		{transaction(2, 100, addr2Key), "addr2SlowTx3"},
		{transaction(3, 100, addr2Key), "addr2SlowTx4"},
	}

	addr3Txs := LabeledTransactions{
		// Enqueue in second promotion
		{transaction(0, 100, addr3Key), "addr3SlowTx1"},
		{transaction(1, 100, addr3Key), "addr3SlowTx2"},
		{transaction(2, 100, addr3Key), "addr3SlowTx3"},
		{transaction(3, 100, addr3Key), "addr3SlowTx4"},
	}

	for _, tx := range addr1Txs {
		fmt.Printf("addr: %s, hash: %s\n", tx.label, tx.tx.Hash())
	}
	...
	slowTxProcessTime := 900 * time.Millisecond
	mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), gomock.Any()).DoAndReturn(func(ctx context.Context, tx *types.Transaction) (sls.ManagerResult, error) {
		time.Sleep(slowTxProcessTime)
	...
	// Explicitly reserve addresses
	pool.reserve(addr1, true)
	pool.reserve(addr2, true)

	// Queue transactions for first promotion
	enqueueTransactions(pool, addr1Txs)
	enqueueTransactions(pool, addr2Txs)

	accounts := []common.Address{addr1, addr2}

	promotables := pool.promoteExecutables(accounts)

	// Maximum 3 workers (MaxConcurrencyPool) can process for this promotion
	// Each call of txprocessor.ProcessTransactionsWithTimeout can only spawn 2 workers (MaxWorkersPerCycle) to process the transactions
	// In this case, 2 slots of semaphore will be occupied by addr1 workers which make 1 slot left for addr2 workers
	if pool.pending[addr1].Len() != pool.slsConfig.MaxWorkersPerCycle {
		t.Errorf("Expect %d number of workers to complete for addr1 due to MaxWorkersPerCycle, got %d", pool.slsConfig.MaxWorkersPerCycle, pool.pending[addr1].Len())
	}
	workerSlotLeft := pool.slsConfig.MaxConcurrencyPool - pool.slsConfig.MaxWorkersPerCycle
	if pool.pending[addr2].Len() != workerSlotLeft {
		t.Errorf("Expect %d number of workers to complete for addr2 due  to MaxWorkersPerCycle, got %d", workerSlotLeft, pool.pending[addr2].Len())
	}
	if len(promotables) != pool.slsConfig.MaxConcurrencyPool {
		t.Errorf("Expect %d number of workers to complete the process, got %d", pool.slsConfig.MaxConcurrencyPool, len(promotables))
	}

	// Wait for the workers to finish processing the remaining transactions
	time.Sleep(2 * time.Second)

	pool.reserve(addr3, true)
	enqueueTransactions(pool, addr3Txs)
	promotables = pool.promoteExecutables([]common.Address{addr3})

	// All transactions for addr1 and addr2 should be completed (workers were spwaned in previous promotion)
	if pool.pending[addr1].Len() != len(addr1Txs) {
		t.Errorf("Expect all transactions are processed for addr1, got %d", pool.pending[addr1].Len())
	}
	if pool.pending[addr2].Len() != len(addr2Txs) {
		t.Errorf("Expect all transactions are processed for addr2, got %d", pool.pending[addr2].Len())
	}
	// The semaphore slots are empty now since all workers of addr1 and addr2 are completed
	// Only 2 workers (MaxWorkersPerCycle) are allowed to process for addr3
	if pool.pending[addr3].Len() != pool.slsConfig.MaxWorkersPerCycle {
		t.Errorf("Expect %d transactions are processed for addr3 due to MaxWorkersPerCycle, got %d", pool.slsConfig.MaxWorkersPerCycle, pool.pending[addr3].Len())
	}
}

func setupMocks(
	delay time.Duration,
	mockManager *sls.MockDetectorManager,
	mockQuarantiner *sls.MockSlsQuarantiner,
	slowTxs types.Transactions,
	quarantineTxs types.Transactions,
	errorTxs types.Transactions,
) {
	// Inject delay to all slow transactions
	for _, slowTx := range slowTxs {
		mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), slowTx).DoAndReturn(func(ctx context.Context, tx *types.Transaction) (sls.ManagerResult, error) {
			time.Sleep(delay)
			return sls.ManagerResult{
				ShouldBeQuarantined: false,
			}, nil
	...
	// Handle all quarantine transactions
	for _, quarantineTx := range quarantineTxs {
		mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), quarantineTx).DoAndReturn(func(ctx context.Context, tx *types.Transaction) (sls.ManagerResult, error) {
			return sls.ManagerResult{
				ShouldBeQuarantined: true,
			}, nil
		})
		mockQuarantiner.EXPECT().SendToQuarantine(gomock.Any(), quarantineTx, gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
	}

	// Handle all error transactions
	for _, errorTx := range errorTxs {
		mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), errorTx).DoAndReturn(func(ctx context.Context, tx *types.Transaction) (sls.ManagerResult, error) {
			return sls.ManagerResult{
				ShouldBeQuarantined: true,
			}, nil
		})
		mockQuarantiner.EXPECT().SendToQuarantine(gomock.Any(), errorTx, gomock.Any(), gomock.Any(), gomock.Any()).Return(errors.New("Error when trying to quarantine transaction"))
	}

	// General expectations for any other transactions
	mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), gomock.Any()).AnyTimes()
	mockManager.EXPECT().Stop().AnyTimes()
}

func enqueueTransactions(pool *LegacyPool, txs LabeledTransactions) {
	for _, tx := range txs {
		pool.enqueueTx(tx.tx.Hash(), tx.tx, false, true)
	}
}

func testPromotionCycle(
	t *testing.T,
	pool *LegacyPool,
	promotionNumber int,
	slsPromotionTimeout time.Duration,
	...
	t.Logf("Running promotion cycle %d", promotionNumber)

	startTime := time.Now()
	pool.promoteExecutables(accounts)
	elapsedTime := time.Since(startTime)

	if shouldWaitForTimeout {
		// Expect that the promotion cycle waits for the timeout because of slow transactions
		if elapsedTime < slsPromotionTimeout {
			t.Errorf("should have waited before collecting promotables, elapsed time: %v", elapsedTime)
		}
	} else {
		// Expect that the promotion cycle completes before the timeout
		if elapsedTime >= slsPromotionTimeout {
			t.Errorf("should collect promotables before timeout, elapsed time: %v", elapsedTime)
		}
	}

	if len(pool.pending) != len(accounts) {
		t.Errorf("expected pending has %d address as key, got %d", len(accounts), len(pool.pending))
	}

	// Expect each address in the pool should have the correct transactions
	for addr, length := range expectedLengths {
		if pool.pending[addr].Len() != length {
			t.Errorf("expected pending tx list length from %s to be %d, got %d", addr.Hex(), length, pool.pending[addr].Len())
		}
		verifyTransactionsInPool(t, pool.pending[addr].Flatten(), expectedTxs[addr])
	}

	// Check for errored transactions being added back to pool
	for _, txHash := range expectedErrorTxs {
		if pool.all.Get(txHash) == nil {
			t.Errorf("errored transaction %s should be added back to the pool", txHash.Hex())
		}
	}
}

func verifyTransactionsInPool(t *testing.T, poolTxs []*types.Transaction, expectedTxs map[common.Hash]string) {
	for _, tx := range poolTxs {
		if label, found := expectedTxs[tx.Hash()]; !found {
			t.Errorf("unexpected transaction in pending list: %s (hash: %s)", label, tx.Hash().Hex())
		}
		// Remove the found hash from the expected map
		delete(expectedTxs, tx.Hash())
	}
	if len(expectedTxs) > 0 {
		for hash, label := range expectedTxs {
			t.Errorf("missing transaction in pool.pending: %s (hash: %s)", label, hash.Hex())
		}
	}
}
...
func testQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	pool := New(config, blockchain, sls.DisabledConfig)
	pool.Init(new(big.Int).SetUint64(testTxPoolConfig.PriceLimit), blockchain.CurrentBlock(), makeAddressReserver())
	defer pool.Close()
...
func TestMinGasPriceEnforced(t *testing.T) {
	...
	pool := New(txPoolConfig, blockchain, sls.DisabledConfig)
