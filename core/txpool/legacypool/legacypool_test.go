...
func validatePoolInternals(pool *LegacyPool) error {
	...
	for addr, hashes := range pool.all.auths {
		for _, hash := range hashes {
			if tx := pool.all.GetRemote(hash); tx == nil {
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetNonce(c.address, 2, tracing.NonceChangeEoACall)
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	blockchain := &testChain{newTestBlockChain(params.TestChainConfig, 1000000000, statedb), address, &trigger}

	tx0 := transaction(0, 100000, key)
	tx1 := transaction(1, 100000, key)

	pool := New(testTxPoolConfig, blockchain, sls.DisabledConfig, sls.DisabledRefreshables)
	pool.Init(context.Background(), testTxPoolConfig.PriceLimit, blockchain.CurrentBlock(), newReserver())
	defer pool.Close()
...
func testSetNonce(pool *LegacyPool, addr common.Address, nonce uint64) {
	pool.mu.Lock()
	pool.currentState.SetNonce(addr, nonce, tracing.NonceChangeEoACall)
	pool.mu.Unlock()
}
...
func TestQueue(t *testing.T) {
	...
	<-pool.requestPromoteExecutables(newAccountSet(pool.signer, from))
	if pool.pending[from].txs.Get(tx.Nonce()) != nil {
		t.Error("expected transaction to not be in tx pool")
	}
	if len(pool.queue) > 0 {
...
func setupMocks(
	delay time.Duration,
	mockManager *slsCommon.MockDetectorManager,
	mockQuarantiner *slsCommon.MockQuarantiner,
	...
	for _, slowTx := range slowTxs {
		mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), slowTx, gomock.Any()).DoAndReturn(func(ctx context.Context, tx *types.Transaction, detectorType slsCommon.DetectorType) (slsCommon.ManagerResult, error) {
			time.Sleep(delay)
			return slsCommon.ManagerResult{
	...
	for _, quarantineTx := range quarantineTxs {
		mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), quarantineTx, gomock.Any()).DoAndReturn(func(ctx context.Context, tx *types.Transaction, detectorType slsCommon.DetectorType) (slsCommon.ManagerResult, error) {
			return slsCommon.ManagerResult{
				ShouldBeQuarantined: true,
			}, nil
		})
		mockQuarantiner.EXPECT().SendToQuarantine(gomock.Any(), quarantineTx, model.PoolQuarantineType, gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
	}

	// Handle all error transactions
	for _, errorTx := range errorTxs {
		mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), errorTx, gomock.Any()).DoAndReturn(func(ctx context.Context, tx *types.Transaction, detectorType slsCommon.DetectorType) (slsCommon.ManagerResult, error) {
			return slsCommon.ManagerResult{
				ShouldBeQuarantined: true,
			}, nil
		})
		mockQuarantiner.EXPECT().SendToQuarantine(gomock.Any(), errorTx, model.PoolQuarantineType, gomock.Any(), gomock.Any(), gomock.Any()).Return(errors.New("Error when trying to quarantine transaction"))
	}

	// General expectations for any other transactions
	mockManager.EXPECT().ShouldBeQuarantined(gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes()
	mockManager.EXPECT().Stop().AnyTimes()
	mockQuarantiner.EXPECT().WasRecentlyQuarantined(gomock.Any()).AnyTimes()
}
...
func testPromotionCycle(
	...
	// Promote the transactions into SLS
	pool.promoteExecutables(accounts)
	// Wait some time for transactions to be processed by SLS worker
	time.Sleep(slsPromotionTimeout)
	// Gather handled transactions with new promotion call
	pool.promoteExecutables(accounts)
...
func TestTipAboveFeeCap(t *testing.T) {
	...
	pool, key := SetupTestPoolWithConfig(eip1559Config)
	defer pool.Close()
...
func TestChainFork(t *testing.T) {
	...
	resetState := func() {
		...
		pool.chain = newTestBlockChain(pool.chainconfig, 1000000, statedb)
		<-pool.requestReset(nil, nil)
	}
	resetState()
...
func TestDoubleNonce(t *testing.T) {
	...
	if tx := pool.pending[addr].txs.Get(0); tx.Hash() != tx2.Hash() {
...
func TestDropping(t *testing.T) {
	...
	if pool.pending[account].txs.Get(tx0.Nonce()) == nil {
		t.Errorf("funded pending transaction missing: %v", tx0)
	}
	if pool.pending[account].txs.Get(tx1.Nonce()) == nil {
		t.Errorf("funded pending transaction missing: %v", tx0)
	}
	if pool.pending[account].txs.Get(tx2.Nonce()) != nil {
		t.Errorf("out-of-fund pending transaction present: %v", tx1)
	}
	if pool.queue[account].txs.Get(tx10.Nonce()) == nil {
		t.Errorf("funded queued transaction missing: %v", tx10)
	}
	if pool.queue[account].txs.Get(tx11.Nonce()) == nil {
		t.Errorf("funded queued transaction missing: %v", tx10)
	}
	if pool.queue[account].txs.Get(tx12.Nonce()) != nil {
...
func TestPostponing(t *testing.T) {
	...
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting())
	blockchain := newTestBlockChain(params.TestChainConfig, 1000000, statedb)
	...
	// Add a batch consecutive pending transactions for txpool
	...
	if pool.pending[accs[0]].txs.Get(txs[0].Nonce()) == nil {
		t.Errorf("tx %d: valid and funded transaction missing from pending pool: %v", 0, txs[0])
	}
	if pool.queue[accs[0]].txs.Get(txs[0].Nonce()) != nil {
		t.Errorf("tx %d: valid and funded transaction present in future queue: %v", 0, txs[0])
	}
	for i, tx := range txs[1:100] {
		if i%2 == 1 {
			if pool.pending[accs[0]].txs.Get(tx.Nonce()) != nil {
				t.Errorf("tx %d: valid but future transaction present in pending pool: %v", i+1, tx)
			}
			if pool.queue[accs[0]].txs.Get(tx.Nonce()) == nil {
	...
	for i, tx := range txs[100:] {
		if i%2 == 1 {
			if pool.queue[accs[1]].txs.Get(tx.Nonce()) == nil {
...
func testQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	pool := New(config, blockchain, sls.DisabledConfig, sls.DisabledRefreshables)
	pool.Init(context.Background(), testTxPoolConfig.PriceLimit, blockchain.CurrentBlock(), newReserver())
	defer pool.Close()
...
func testQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	pool := New(config, blockchain, sls.DisabledConfig, sls.DisabledRefreshables)
	pool.Init(context.Background(), config.PriceLimit, blockchain.CurrentBlock(), newReserver())
	defer pool.Close()
	...
	statedb.SetNonce(crypto.PubkeyToAddress(remote.PublicKey), 2, tracing.NonceChangeEoACall)
	statedb.SetNonce(crypto.PubkeyToAddress(local.PublicKey), 2, tracing.NonceChangeEoACall)
	<-pool.requestReset(nil, nil)
...
func TestPendingGlobalLimiting(t *testing.T) {
	...
	for _, key := range keys {
		addr := crypto.PubkeyToAddress(key.PublicKey)
		for range int(config.GlobalSlots) / len(keys) * 2 {
...
func TestCapClearsFromAll(t *testing.T) {
	...
	txs := types.Transactions{}
	for j := range int(config.GlobalSlots) * 2 {
...
func TestPendingMinimumAllowance(t *testing.T) {
	...
	for _, key := range keys {
		addr := crypto.PubkeyToAddress(key.PublicKey)
		for range int(config.AccountSlots) * 2 {
			txs = append(txs, transaction(nonces[addr], 100000, key))
			nonces[addr]++
		}
	}
...
func TestMinGasPriceEnforced(t *testing.T) {
	...
	// Create the pool to test the pricing enforcement with
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting())
	blockchain := newTestBlockChain(eip1559Config, 10000000, statedb)

	txPoolConfig := DefaultConfig
	txPoolConfig.NoLocals = true
	pool := New(txPoolConfig, blockchain, sls.DisabledConfig, sls.DisabledRefreshables)
	pool.Init(context.Background(), txPoolConfig.PriceLimit, blockchain.CurrentBlock(), newReserver())
	defer pool.Close()
...
func TestRepricingDynamicFee(t *testing.T) {
	...
	// Create the pool to test the pricing enforcement with
	pool, _ := SetupTestPoolWithConfig(eip1559Config)
	defer pool.Close()
...
func TestRepricingKeepsLocals(t *testing.T) {
	...
	// Create the pool to test the pricing enforcement with
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting())
	blockchain := newTestBlockChain(eip1559Config, 1000000, statedb)
...
func testJournaling(t *testing.T, nolocals bool, journalRemotes bool) {
	...
	pool := New(config, blockchain, sls.DisabledConfig, sls.DisabledRefreshables)
	if err := pool.Init(context.Background(), config.PriceLimit, blockchain.CurrentBlock(), newReserver()); err != nil {
		t.Error(err)
	}
	...
	pool.Close()
	statedb.SetNonce(crypto.PubkeyToAddress(local.PublicKey), 1, tracing.NonceChangeEoACall)
	blockchain = newTestBlockChain(params.TestChainConfig, 1000000, statedb)
...
func TestSetCodeTransactions(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		pending int
		queued  int
		run     func(name string, pool *LegacyPool, statedb *state.StateDB)
	}{
		{
		...
		blockchain := newTestBlockChain(params.MergedTestChainConfig, 1000000000, statedb)
		pool := New(testTxPoolConfig, blockchain, sls.DisabledConfig, sls.DisabledRefreshables)
		pool.Init(context.Background(), testTxPoolConfig.PriceLimit, blockchain.CurrentBlock(), newReserver())
		defer pool.Close()
...
func TestSetCodeTransactionsReorg(t *testing.T) {
	...
	// Create the pool to test the status retrievals with
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting())
	blockchain := newTestBlockChain(params.MergedTestChainConfig, 1000000, statedb)
	...
	blockchain.statedb.SetNonce(addrA, 1, tracing.NonceChangeEoACall)
	...
	// Simulate the chain moving
	blockchain.statedb.SetNonce(addrA, 2, tracing.NonceChangeEoACall)
...
func TestLegacyPool_Init(t *testing.T) {
	t.Parallel()
	...
	// test: sls disabled
	pool := New(testTxPoolConfig, blockchain, sls.DisabledConfig, sls.DisabledRefreshables)
	err := pool.Init(t.Context(), testTxPoolConfig.PriceLimit, blockchain.CurrentBlock(), newReserver())
	assert.NoError(t, err)

	// assert that sls txprocessor is nil
	assert.Nil(t, pool.txprocessor)
	pool.Close()

	// test: sls enabled
	slsConfig := sls.Config{Refreshable: sls.RefreshableConfigs{Enabled: true}}
	refreshableGetter := sls.GetterOrStatic(slsConfig.Refreshable)
	pool = New(testTxPoolConfig, blockchain, slsConfig, refreshableGetter)

	pool.detectorManager = slsCommon.NewMockDetectorManager(ctrl)
	pool.quarantiner = slsCommon.NewMockQuarantiner(ctrl)
	pool.executor = sls.NewExecutor(pool.quarantiner, pool.signer, pool)

	err = pool.Init(t.Context(), testTxPoolConfig.PriceLimit, blockchain.CurrentBlock(), newReserver())
	assert.NoError(t, err)

	if pool.GetSLSRefreshables().Enabled && pool.slsConfig.IsEnabled() {
		assert.NotNil(t, pool.txprocessor)
	}
...
type stubReserver struct {
	f func(addr common.Address, take bool) error
}

func (s *stubReserver) Hold(addr common.Address) error    { return s.f(addr, true) }
func (s *stubReserver) Release(addr common.Address) error { return s.f(addr, false) }
func (s *stubReserver) Has(addr common.Address) bool      { return true }
