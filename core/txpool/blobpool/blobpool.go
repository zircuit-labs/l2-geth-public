...
func (p *BlobPool) Init(gasTip *big.Int, head *types.Header, reserve txpool.AddressReserver) error {
	...
	// fully synced).
	state, err := p.chain.StateAtHeader(head)
	if err != nil {
...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	statedb, err := p.chain.StateAtHeader(newHead)
	if err != nil {
...
func (p *BlobPool) RemoveTx(hash common.Hash, outofbound bool, unreserve bool) {
	// This doesn't really make sense for the blobpool
