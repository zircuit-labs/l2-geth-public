...
func TestOpenDrops(t *testing.T) {
	...
	if err := pool.Init(context.Background(), 1, chain.CurrentBlock(), newReserver()); err != nil {
...
func TestBlobCountLimit(t *testing.T) {
	...
	errs := pool.Add([]*types.Transaction{tx1, tx2}, true, false)
	...
	for i, tt := range tests {
		...
		if tt.block != nil {
			...
			chain.blocks = map[uint64]*types.Block{
				header.Number.Uint64(): types.NewBlockWithHeader(header).WithBody(txs, nil),
			}
