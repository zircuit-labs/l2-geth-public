...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/common/lru"
	"github.com/zircuit-labs/l2-geth-public/common/mclock"
	...
	"github.com/zircuit-labs/l2-geth-public/internal/syncx"
	"github.com/zircuit-labs/l2-geth-public/internal/version"
...
func (c *CacheConfig) triedbConfig(isZktrie bool) *trie.Config {
	config := &trie.Config{Preimages: c.Preimages, IsZktrie: isZktrie}
...
type BlockChain struct {
	...
	zkMigrator  *ZKStateMigrator
	zkTrieCache *trie.Database // to open ZKTrie StateDB, nil means migration hasn't finished
}
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	trieConfig := cacheConfig.triedbConfig(params.ZKTrieEnabled.Load())
	// override snapshot setting
	if params.ZKTrieEnabled.Load() && cacheConfig.SnapshotLimit > 0 {
	...
	// Set up the genesis block, commit the provided genesis specification
	// to database if the genesis block is not present yet, or load the
	// stored one from database.
	chainConfig, genesisHash, genesisErr := SetupGenesisBlockWithOverride(db, triedb, genesis, overrides)
	if genesisErr != nil && !errors.As(genesisErr, &(*new(*params.ConfigCompatError))) {
	...
	if chainConfig != nil && chainConfig.IsOptimism() && chainConfig.RegolithTime == nil {
	...
	bc.validator = NewBlockValidator(chainConfig, bc, engine, db)
	bc.prefetcher = newStatePrefetcher(chainConfig, bc, engine)
	bc.processor = NewStateProcessor(chainConfig, bc, engine)

	if cc := chainConfig; cc != nil && cc.ZKTrieSwitchBlock != nil && cc.ZKTrieSwitchBlock.Uint64() > 0 {
		bc.zkMigrator = NewZKStateMigrator(db, bc, genesis, cc.ZKTrieSwitchBlock.Uint64()).
	...
	if !bc.HasStateWithNum(head.Root, head.Number.Uint64()) {
	...
	if bc.zkMigrator != nil {
		go func() {
			if err = bc.zkMigrator.Start(context.Background()); err != nil {
				log.Error("Failed to migrate state to ZKTrie", "err", err)
			}
		}()
	}
	return bc, nil
		...
		if currentBlock := bc.CurrentBlock(); currentBlock != nil && header.Number.Uint64() <= currentBlock.Number.Uint64() {
			...
			if newHeadBlock == nil {
				...
				for {
					...
					if !bc.HasStateWithNum(newHeadBlock.Root(), newHeadBlock.NumberU64()) &&
						!bc.stateRecoverable(newHeadBlock.Root()) {
					...
					if beyondRoot || newHeadBlock.NumberU64() == 0 {
						if !bc.HasStateWithNum(newHeadBlock.Root(), newHeadBlock.NumberU64()) && bc.stateRecoverable(newHeadBlock.Root()) {
...
func (bc *BlockChain) SnapSyncCommitHead(hash common.Hash) error {
	...
	if !bc.HasStateWithNum(root, block.NumberU64()) {
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		var (
			root      = bc.HeaderOrWaitZKTrie(parent).Root
			cachingDB = bc.stateCache
		)
		if block != nil && bc.isZkTrieSwitch(block.NumberU64()) {
			if bc.zkTrieCache == nil {
				return 0, fmt.Errorf("expected ZKTrie migration finished on block %d", block.NumberU64())
			}
			cachingDB = state.NewDatabaseWithNodeDB(bc.db, bc.zkTrieCache)
		}

		statedb, err := state.New(root, cachingDB, bc.snaps)
		if err != nil {
			return it.index, err
		}
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator) (int, error) {
	...
	parent := it.previous()
	for parent != nil && !bc.HasStateWithNum(parent.Root, parent.Number.Uint64()) {
...
func (bc *BlockChain) recoverAncestors(block *types.Block) (common.Hash, error) {
	...
	for parent != nil && !bc.HasStateWithNum(parent.Root(), parent.NumberU64()) {
		if bc.stateRecoverable(parent.Root()) {
			if err := bc.triedb.Recover(parent.Root()); err != nil {
...
func (bc *BlockChain) SetCanonical(head *types.Block) (common.Hash, error) {
	...
	if !bc.HasStateWithNum(head.Root(), head.NumberU64()) {
