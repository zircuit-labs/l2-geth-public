...
func NewEVMInterpreter(evm *EVM) *EVMInterpreter {
	...
	case evm.chainRules.IsTenrec:
		table = &tenrecInstructionSet
...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	for {
		...
		if in.evm.executionLimiter != nil {
			if err := in.evm.executionLimiter.TrackOpcode(op, cost); err != nil {
				return nil, err
			}
			// Check for limit errors from subcalls (CALL, STATICCALL, etc.)
			if in.evm.limitError != nil {
				return nil, in.evm.limitError
			}
		}
		if memorySize > 0 {
