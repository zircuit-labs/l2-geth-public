...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/core/asm"
...
func TestCall(t *testing.T) {
	state, _ := state.New(types.EmptyLegacyTrieRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
...
func benchmarkNonModifyingCode(gas uint64, code []byte, name string, tracerCode string, b *testing.B) {
	cfg := new(Config)
	setDefaults(cfg)
	cfg.State, _ = state.New(types.EmptyLegacyTrieRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	cfg.GasLimit = gas
	if len(tracerCode) > 0 {
...
// correctly
// see: https://github.com/zircuit-labs/l2-geth-public/issues/22649
