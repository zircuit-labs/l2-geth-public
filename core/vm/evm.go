...
// ExecutionLimiter represents a strategy for enforcing execution limits within the EVM.
...
type EVM struct {
	...
	limitError       error // Track limit errors from subcalls
...
func (evm *EVM) SetTxContext(txCtx TxContext) {
	...
	evm.TxContext = txCtx
	evm.limitError = nil
	if evm.executionLimiter != nil {
		evm.executionLimiter.ResetTx()
	}
}
...
// SetExecutionLimiter sets the executionLimiter for the EVM.
// This limiter is only used for miner.
func (evm *EVM) SetExecutionLimiter(executionLimiter ExecutionLimiter) {
	evm.executionLimiter = executionLimiter
}

// ExecutionLimiter returns the current ExecutionLimiter
func (evm *EVM) ExecutionLimiter() ExecutionLimiter {
	return evm.executionLimiter
}
...
func (evm *EVM) executePrecompile(addr common.Address, p PrecompiledContract, input []byte, suppliedGas uint64) ([]byte, uint64, error) {
	output, remaining, err := RunPrecompiledContract(p, input, suppliedGas, evm.Config.Tracer)
	if evm.executionLimiter != nil {
		gasUsed := suppliedGas - remaining
		if err := evm.executionLimiter.TrackPrecompile(addr, gasUsed); err != nil {
			// Store the limit error so it can be checked in the interpreter loop
			evm.limitError = err
...
func (evm *EVM) Call(caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if isPrecompile {
		ret, gas, err = evm.executePrecompile(addr, p, input, gas)
	} else {
