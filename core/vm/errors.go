...
type LimiterScope string

const (
	LimiterTxScope    LimiterScope = "transaction"
	LimiterBlockScope LimiterScope = "block"
)

type ErrOpcodeLimit struct {
	Opcode OpCode
	Scope  LimiterScope
	Limit  uint64
}

func (e *ErrOpcodeLimit) Error() string {
	return fmt.Sprintf("%s opcode limit exceeded (%s cap %d)", e.Opcode, e.Scope, e.Limit)
}

type ErrPrecompileLimit struct {
	Address common.Address
	Scope   LimiterScope
	Limit   uint64
}

func (e *ErrPrecompileLimit) Error() string {
	return fmt.Sprintf("%s precompile limit exceeded (%s cap %d)", e.Address.Hex(), e.Scope, e.Limit)
}
...
const (
	...
	VMErrorCodeOpcodeLimit
	VMErrorCodePrecompileLimit
...
func vmErrorCodeFromErr(err error) int {
	...
	default:
		...
		if v := (*ErrOpcodeLimit)(nil); errors.As(err, &v) {
			return VMErrorCodeOpcodeLimit
		}

		if v := (*ErrPrecompileLimit)(nil); errors.As(err, &v) {
			return VMErrorCodePrecompileLimit
		}
		return VMErrorCodeUnknown
	}
}

func IsEVMBlockLimitError(err error) bool {
	var (
		opLimit  *ErrOpcodeLimit
		preLimit *ErrPrecompileLimit
	)
	if errors.As(err, &opLimit) && opLimit.Scope == LimiterBlockScope {
		return true
	}
	if errors.As(err, &preLimit) && preLimit.Scope == LimiterBlockScope {
		return true
	}
	return false
}

func IsEVMTxLimitError(err error) bool {
	var (
		opLimit  *ErrOpcodeLimit
		preLimit *ErrPrecompileLimit
	)
	if errors.As(err, &opLimit) && opLimit.Scope == LimiterTxScope {
		return true
	}
	if errors.As(err, &preLimit) && preLimit.Scope == LimiterTxScope {
		return true
	}
	return false
}

func IsEVMLimitError(err error) bool {
	var (
		opLimit  *ErrOpcodeLimit
		preLimit *ErrPrecompileLimit
	)
	return errors.As(err, &opLimit) || errors.As(err, &preLimit)
}
