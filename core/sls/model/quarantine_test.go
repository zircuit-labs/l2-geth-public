...
func TestNewQuarantineNormal(t *testing.T) {
	...
	quarantine, err := NewQuarantine(tx, detector, reason, from.String(), duration, 0)
	...
	assert.Equal(t, tx.Value().Uint64(), quarantine.Value)
...
func TestNewQuarantineRejected(t *testing.T) {
	...
	assert.False(t, quarantine.IsReleased)
	assert.Equal(t, loss, quarantine.Loss)
}
...
func TestTx(t *testing.T) {
	...
	quarantine, err := NewQuarantine(tx, detector, reason, from.String(), time.Second, 0)
	assert.NoError(t, err)
