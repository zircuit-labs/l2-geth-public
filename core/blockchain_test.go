...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/common/math"
	"github.com/zircuit-labs/l2-geth-public/consensus"
	"github.com/zircuit-labs/l2-geth-public/consensus/beacon"
...
func newCanonical(engine consensus.Engine, n int, full bool, scheme string) (ethdb.Database, *Genesis, *BlockChain, error) {
	...
	if full {
		...
		for _, block := range blocks {
			rawdb.WriteCanonicalHash(genDb, block.Hash(), block.NumberU64())
			rawdb.WriteBlock(genDb, block)
		}
...
func testEIP155Transition(t *testing.T, scheme string) {
	...
	_, err := blockchain.InsertChain(blocks)
	// Due to use of CCC the transaction tracing fails before getting and invalid chain id
	// however we can still ensure that there was some kind of error so this test remains useful.
	// if have, want := err, types.ErrInvalidChainId; !errors.Is(have, want) {
	// 	t.Errorf("have %v, want %v", have, want)
	// }
	if err == nil {
		t.Errorf("have %v, want: an error of some kind", err)
	}
}
...
//
// https://github.com/zircuit-labs/l2-geth-public/pull/15941
...
//
// Details at:
//   - https://github.com/zircuit-labs/l2-geth-public/issues/18977
//   - https://github.com/zircuit-labs/l2-geth-public/pull/18988
...
func TestPoseidonCodeHash(t *testing.T) {
	...
	t.Cleanup(func() { params.ZKTrieEnabled.Store(false) })
	...
	// 	}
	// }
	deployCode := common.Hex2Bytes("")

	callCreateCode := common.Hex2Bytes("")
	callCreate2Code := common.Hex2Bytes("")
	...
	var (
		...
		signer  = types.LatestSigner(gspec.Config)
		engine  = ethash.NewFaker()
		triedb  = trie.NewDatabase(db, &trie.Config{IsZktrie: true})
	...
	chain, receipts = GenerateChain(params.TestChainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), engine, db, 1, func(i int, gen *BlockGen) {
...
func TestBlockPayloadSizeLimit(t *testing.T) {
	...
	*config.Scroll.MaxTxPayloadBytesPerBlock = 150
	...
	// Insert empty block
	block1, _ := GenerateChain(gspec.Config, genesis, engine, db, 1, func(i int, gen *BlockGen) {
		// empty
	})
	...
	block2, _ := GenerateChain(gspec.Config, genesis, engine, db, 1, func(i int, gen *BlockGen) {
		addTx(gen)
	})
	...
	block3, _ := GenerateChain(gspec.Config, genesis, engine, db, 1, func(i int, gen *BlockGen) {
		addTx(gen)
		addTx(gen)
	})
