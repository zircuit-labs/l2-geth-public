...
func (b *BlockGen) collectRequests(readonly bool) (requests [][]byte) {
	...
	isTenrec := b.cm.config.IsTenrec(b.header.Time)

	if b.cm.config.IsPrague(b.header.Number, b.header.Time) && !isTenrec {
		requests = [][]byte{}
		// EIP-6110 deposits
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db ethdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	...
	genblock := func(i int, parent *types.Block, triedb *triedb.Database, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		if config.IsTenrec(b.header.Time) {
		...
		if requests != nil && config.IsTenrec(b.header.Time) {
	...
	for i := range n {
		...
		l1Info := rawdb.ReadL1Info(db, block.Hash())
		...
		if err := receipts.DeriveFields(config, block.Hash(), block.NumberU64(), block.Time(), block.BaseFee(), blobGasPrice, txs, l1Info); err != nil {
			panic(err)
		}
...
func (cm *chainMaker) makeHeader(parent *types.Block, state *state.StateDB, engine consensus.Engine) *types.Header {
	...
	if cm.config.IsCancun(header.Number, header.Time) {
		excessBlobGas := eip4844.CalcExcessBlobGas(cm.config, parent.Header(), time)
		header.ExcessBlobGas = &excessBlobGas
		header.BlobGasUsed = new(uint64)
		if cm.config.IsTenrec(header.Time) {
