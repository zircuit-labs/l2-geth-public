...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/trie/utils"
)
...
type Database interface {
	...
	CopyTrie(Trie) Trie
	Copy() Database
	WithTrieDB(*trie.Database) Database
...
func (db *cachingDB) OpenTrie(root common.Hash) (Trie, error) {
	...
	if db.triedb.IsZktrie() {
		return trie.NewZkTrie(root, trie.NewZktrieDatabaseFromTriedb(db.triedb))
	}
...
func (db *cachingDB) Copy() Database {
	return &cachingDB{
		disk:          db.disk,
		codeSizeCache: db.codeSizeCache,
		codeCache:     db.codeCache,
		triedb:        db.triedb.Copy(),
	}
}
