...
func (s *stateObject) getTrie() (Trie, error) {
	if s.trie == nil {
		...
		if !types.IsEmptyTrieRoot(s.data.Root) && s.db.prefetcher != nil {
...
func (s *stateObject) GetCommittedState(key common.Hash) common.Hash {
	...
	if _, destructed := s.db.stateObjectsDestruct.Load(s.address); destructed {
		return common.Hash{}
	}
...
func (s *stateObject) finalise(prefetch bool) {
	...
	if s.db.prefetcher != nil && prefetch && len(slotsToPrefetch) > 0 &&
		s.data.Root != types.EmptyLegacyTrieRootHash && s.data.Root != types.EmptyZkTrieRootHash {
...
func (s *stateObject) updateTrie() (Trie, error) {
	...
	var (
		storage *xsync.MapOf[common.Hash, []byte]
		origin  *xsync.MapOf[common.Hash, []byte]
	)
	tr, err := s.getTrie()
	if err != nil {
		s.db.setError(err)
	...
	for key, value := range s.pendingStorage {
		...
		if storage == nil {
			if storage, _ = s.db.storages.Load(s.addrHash); storage == nil {
				storage = xsync.NewMapOf[common.Hash, []byte]()
				s.db.storages.Store(s.addrHash, storage)
			}
		}
		khash := crypto.HashData(s.db.hasher, key[:])
		storage.Store(khash, encoded) // encoded will be nil if it's deleted

		// Cache the original value of mutated storage slots
		if origin == nil {
			if origin, _ = s.db.storagesOrigin.Load(s.address); origin == nil {
				origin = xsync.NewMapOf[common.Hash, []byte]()
				s.db.storagesOrigin.Store(s.address, origin)
			}
		}
		// Track the original value of slot only if it's mutated first time
		if _, ok := origin.Load(khash); !ok {
			if prev == (common.Hash{}) {
				origin.Store(khash, nil) // nil if it was not present previously
			} else {
				// Encoding []byte cannot fail, ok to ignore the error.
				b, _ := rlp.EncodeToBytes(common.TrimLeftZeroes(prev[:]))
				origin.Store(khash, b)
			}
		}
...
func (s *stateObject) CodeSize() uint64 {
	// code size is stored directly when zktrie is enabled
	if s.isZkTrie() {
...
func (s *stateObject) setCode(code []byte) {
	s.code = code
	s.data.KeccakCodeHash = crypto.Keccak256Hash(code).Bytes()
	if s.isZkTrie() {
		s.data.PoseidonCodeHash = poseidon.CodeHash(code).Bytes()
...
func (s *stateObject) isZkTrie() bool {
	_, ok := s.db.trie.(*trie.ZkTrie)
	return ok
}
