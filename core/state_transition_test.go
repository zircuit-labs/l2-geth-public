...
func TestValidateAuthorizations(t *testing.T) {
	...
	evm := vm.NewEVM(vm.BlockContext{BlockNumber: new(big.Int), Time: new(big.Int).Uint64()}, stateDb, params.TestChainConfig, vm.Config{})
	...
	t.Run("Chain ID mismatch", func(t *testing.T) {
		...
		_, err := st.validateAuthorization(auth)
	...
	t.Run("Destination has code", func(t *testing.T) {
		...
		patches.ApplyMethod(reflect.TypeOf(st.state),
			"GetCode",
			func(_ any, addr common.Address) []byte {
	...
	t.Run("Nonce mismatch", func(t *testing.T) {
		...
		patches.ApplyMethod(reflect.TypeOf(st.state),
			"GetNonce",
			func(_ any, addr common.Address) uint64 {
				return 1
			},
		)
		auth := &types.SetCodeAuthorization{
