...
func NewBlockValidator(
	config *params.ChainConfig, blockchain *BlockChain) *BlockValidator {
...
func (v *BlockValidator) ValidateBody(block *types.Block) error {
	...
	if !v.config.IsAlfie(block.Time()) {
	...
	if header.WithdrawalsHash != nil {
		...
		if v.config.IsTenrec(header.Time) {
...
func (v *BlockValidator) ValidateState(block *types.Block, statedb *state.StateDB, res *ProcessResult, stateless bool) error {
	...
	if v.config.IsTenrec(block.Time()) {
		if header.WithdrawalsHash == nil {
			return errors.New("expected withdrawals root in Zircuit post-Tenrec block header")
		}
		// Validate the withdrawals root against the L2 withdrawals root storage slot
		if root := statedb.GetState(params.OptimismL2ToL1MessagePasser, params.WithdrawalRootStorageSlot); *header.WithdrawalsHash != root {
			return fmt.Errorf("invalid withdrawal root hash (remote: %s local: %s) dberr: %w", *header.WithdrawalsHash, root, statedb.Error())
		}
	}
	return nil
}
