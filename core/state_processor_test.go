	...
	{ // Tests against a 'recent' chain definition
		...
		for _, tt := range []struct {
			...
			},
			{
				...
				want: "could not apply tx 0 [0xbd49d8dadfd47fb846986695f7d4da3f7b2c48c8da82dbc211a26eb124883de9]: transaction gas limit too high (cap: 16777216, tx: 21000000)",
			},
			{
			...
			{
				name: "ErrMaxInitCodeSizeExceeded",
				txs: []*types.Transaction{
			...
			{
				name: "ErrIntrinsicGas", // Not enough gas to cover init code
				txs: []*types.Transaction{
	...
	// ErrTxTypeNotSupported, For this, we need an older chain
	{
		...
		for i, tt := range []struct {
			...
			{ // ErrTxTypeNotSupported
				...
				want: "could not apply tx 0 [0xc4ab868fef0c82ae0387b742aee87907f2d0fc528fc6ea0a021459fb0fc4a4a8]: transaction type not supported",
			},
		} {
...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions, config *params.ChainConfig, isPOW bool) *types.Block {
	...
	if config.IsCancun(header.Number, header.Time) {
		...
		if config.IsTenrec(header.Time) {
	...
	if config.IsShanghai(header.Number, header.Time) {
		return types.NewBlockWithWithdrawals(header, txs, nil, receipts, []*types.Withdrawal{}, trie.NewStackTrie(nil), config.IsTenrec(header.Time))
	}
