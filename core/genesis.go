...
func (ga *GenesisAlloc) hash(isVerkle, isZkTrie bool) (common.Hash, error) {
	if isZkTrie && isVerkle {
		return common.Hash{}, errors.New("cannot have both zktrie and verkle trie enabled")
	}
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *trie.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
	...
	if params.ZKTrieEnabled.Load() {
		// overwrite triedb IsZktrie config to be safe
		triedb.SetIsZktrie(true)
	}
	...
	if isStateInitRequired(genesis, header.Root, db, triedb) {
		if genesis == nil {
			genesis = DefaultGenesisBlock()
		}
		applyOverrides(genesis.Config)
...
func (g *Genesis) ToBlock() *types.Block {
	...
	if g.StateHash != nil {
		...
		root = *g.StateHash
	} else if root, err = g.Alloc.hash(g.IsVerkle(), params.ZKTrieEnabled.Load()); err != nil {
		panic(err)
	}
	head := &types.Header{
...
func (g *Genesis) Commit(db ethdb.Database, triedb *trie.Database) (*types.Block, error) {
	...
	if params.ZKTrieEnabled.Load() != triedb.IsZktrie() {
		return nil, fmt.Errorf("ZktrieEnabled mismatch: global=%v, triedb=%v", params.ZKTrieEnabled.Load(), triedb.IsZktrie())
	}
...
func DeveloperGenesisBlock(gasLimit uint64, faucet *common.Address) *Genesis {
	...
	if faucet != nil {
		genesis.Alloc[*faucet] = GenesisAccount{Balance: new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 248), big.NewInt(9))}
	}
	return genesis
}
...
// isStateInitRequired is called to check when the genesis block is present
// (perhaps in ancient database) whether the state database is not initialized
// yet. It can happen that the node is initialized with an external ancient
// store. Commit genesis state in this case. If the bedrock block is not 0, that
// implies that the network was migrated at the bedrock block. In this case the
// genesis state may not be in the state database (e.g. op-geth is performing a
// snap sync without an existing datadir) & even if it were, would not be useful
// as op-geth is not able to execute the pre-bedrock STF.
//
// ZKTrie check is different, because it stores genesis root in another place.
// This is how Scroll checks initialization: if DB is successfully created, the
// given root was found, meaning initialization was done.
func isStateInitRequired(genesis *Genesis, root common.Hash, db ethdb.Database, triedb *trie.Database) bool {
	if triedb.IsZktrie() {
		_, err := state.New(root, state.NewDatabaseWithNodeDB(db, triedb), nil)
		return err != nil
	}

	transitionedNetwork := genesis != nil && genesis.Config != nil && genesis.Config.BedrockBlock != nil && genesis.Config.BedrockBlock.Uint64() != 0
	return root != types.EmptyLegacyTrieRootHash && !triedb.Initialized(root) && !transitionedNetwork
}
