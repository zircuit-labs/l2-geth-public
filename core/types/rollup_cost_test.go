...
func TestNewL1CostFunc(t *testing.T) {
	...
	// an edge case where blockTime is equal to forBlock but cachedFunc is not being initialized yet
	// should not panic as we have safe guard for cachedFunc being nil
	costFunc = NewL1CostFunc(config, statedb)
	require.NotNil(t, costFunc)
	fee = costFunc(emptyTx.RollupCostData(), ^uint64(0))
	require.Nil(t, fee)
}
