...
// PayloadSize returns the sum of all L2 transactions in a block.
...
func NewBlockWithWithdrawals(header *Header, txs []*Transaction, uncles []*Header, receipts []*Receipt, withdrawals []*Withdrawal, hasher TrieHasher, Tenrec bool) *Block {
	b := NewBlock(header, txs, uncles, receipts, hasher)

	if Tenrec {
		if withdrawals == nil || len(withdrawals) > 0 {
			panic(fmt.Sprintf("expected non-nil empty withdrawals operation list in Tenrec, but got: %v", withdrawals))
		}
