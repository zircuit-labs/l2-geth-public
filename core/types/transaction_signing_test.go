...
func TestLatestSignerCancun(t *testing.T) {
	...
	// check cancunSigner
	signer, ok = signer.(cancunSigner)
	if ok != true {
		t.Error("expected londonSigner")
	}
}

func TestLatestSignerPrague(t *testing.T) {
	config := params.TestChainConfig
	var genesisTime uint64 = 0
	...
	config.ShanghaiTime = &genesisTime
	config.CancunTime = &genesisTime
	config.PragueTime = &genesisTime

	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)
	addr2 := common.HexToAddress("0x000000000000000000000000000000000000aaaa")
	chainID := big.NewInt(1)

	// check if pragueSigner
	signer := LatestSigner(config)
	signer, ok := signer.(pragueSigner)
	if ok != true {
		t.Error("expected pragueSigner")
	}

	// check if signing setCodeTx works
	auth1, _ := SignSetCode(key, SetCodeAuthorization{ChainID: *uint256.MustFromBig(chainID), Address: addr2, Nonce: 0})
	txdata1 := &SetCodeTx{
		ChainID:   uint256.MustFromBig(chainID),
		Nonce:     0,
		To:        common.Address{},
		Gas:       500000,
		GasFeeCap: uint256.MustFromBig(new(big.Int).Mul(big.NewInt(5), big.NewInt(params.GWei))),
		GasTipCap: uint256.NewInt(2),
		AuthList:  []SetCodeAuthorization{auth1},
	}

	tx, err := SignNewTx(key, signer, txdata1)
	if err != nil {
		t.Error("expected no error")
	}
	from, err := Sender(signer, tx)
	...
	_, err = SignNewTx(key, signer, txdata2)
	if err != nil {
		t.Error("expected no error")
	}

	// enable optimismPragueSigner
	config.Optimism = &params.OptimismConfig{EIP1559Elasticity: 50, EIP1559Denominator: 10}
	signer = LatestSigner(config)
	signer, ok = signer.(optimismPragueSigner)
	if ok != true {
		t.Error("expected optimismPragueSigner")
	}
