...
// DebugBlockTraceResult provides block trace, code map and Proofs of all addresses involved in the block
type DebugBlockTraceResult struct {
	BlockTrace *BlockTrace                      `json:"blockTrace"`
	CodeMap    map[common.Address]hexutil.Bytes `json:"codeMap"`
	Proofs     []*AccountResult                 `json:"proofs"`
}
...
type ExecutionResult struct {
	...
	// List with account info for each contract from `SetCodeTx.AuthList`
	AuthorizationList []*AccountWrapper `json:"authorizationList,omitempty"`
}
...
func NewTransactionData(tx *Transaction, blockNumber, blockTime uint64, config *params.ChainConfig, baseFee *big.Int) *TransactionData {
	...
	if baseFee != nil {
		tx.inner.effectiveGasPrice(gasPrice, baseFee)
	}
