...
//go:generate go tool gencodec -type Receipt -field-override receiptMarshaling -out gen_receipt_json.go
...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash, number uint64, time uint64, baseFee *big.Int, blobGasPrice *big.Int, txs []*Transaction, l1Info *L1Info) error {
	...
	if config.Optimism != nil && len(txs) > 0 && (len(txs) >= 2 || l1Info != nil) && config.IsBedrock(new(big.Int).SetUint64(number)) { // need at least an info tx and a non-info tx
		var (
			gasParams gasParams
			err       error
		)
		if l1Info == nil {
			...
			gasParams, err = extractL1GasParamsFromL1Info(config, time, l1Info)
		}
