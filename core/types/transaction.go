...
// ProtocolTx checks if any transaction is created as part of the protocol.
// removing any such tx can potentially break the protocol
func ProtocolTx(index int, tx *Transaction, sender common.Address) bool {
	return L2AttributesTx(index, tx, sender)
}

// note: isSystemTransaction flag is not set after regolith fork. So, IsSystemTx() returns false for L1AttributesTx
func L2AttributesTx(index int, tx *Transaction, sender common.Address) bool {
	return index == 0 && tx.To() != nil && tx.To().Cmp(L1BlockAddr) == 0 && sender.Cmp(SystemAddress) == 0
}

// AsDepositTx casts the tx into a DepositTx.
func (tx *Transaction) AsDepositTx() *DepositTx {
	if dep, ok := tx.inner.(*DepositTx); ok {
		return dep
	}
	return nil
