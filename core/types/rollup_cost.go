...
func (a RollupCostData) Zeroes() uint64 {
	return a.zeroes
}

func (a RollupCostData) Ones() uint64 {
	return a.ones
}
...
func NewL1CostFunc(config *params.ChainConfig, statedb StateGetter) L1CostFunc {
	...
	return func(rollupCostData RollupCostData, blockTime uint64) *big.Int {
		...
		if cachedFunc == nil {
			log.Error("cachedFunc is nil after initialization", "forBlock", forBlock, "blockTime", blockTime)
			return nil
		}
...
func extractL1GasParams(config *params.ChainConfig, time uint64, data []byte) (l1BaseFee *big.Int, costFunc l1CostFunc, feeScalar *big.Float, err error) {
	if config.IsEcotone(time) {
		...
		if len(data) >= 4 && !bytes.Equal(data[0:4], BedrockL1AttributesSelector) && !bytes.Equal(data[0:4], BedrockL1ExclusionsAttributesSelector) {
...
func extractL1GasParamsEcotone(data []byte) (l1BaseFee *big.Int, costFunc l1CostFunc, err error) {
	if bytes.Equal(data[0:4], EcotoneL1AttributesSelector) {
		...
		if len(data) < 164 {
			return nil, nil, fmt.Errorf("expected at least 164 L1 info bytes, got %d", len(data))
		}
	} else {
		return nil, nil, fmt.Errorf("unexpected function signature in gas extraction %v", data[0:4])
	}
