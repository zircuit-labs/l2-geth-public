...
func (s *StateAccount) MarshalFields() ([]zkt.Byte32, uint32) {
	...
	if !utils.CheckBigIntInField(s.Balance.ToBig()) {
		panic("StateAccount balance overflow")
	}

	if !utils.CheckBigIntInField(s.Root.Big()) {
		panic(fmt.Sprintf("StateAccount root overflow: %s", s.Root.String()))
	}
	...
	s.Balance.ToBig().FillBytes(fields[1][:])
...
func UnmarshalStateAccount(bytes []byte) (*StateAccount, error) {
	...
	acc.Balance = new(uint256.Int).SetBytes(bytes[32:64])
