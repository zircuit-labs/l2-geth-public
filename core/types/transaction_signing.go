...
func MakeSigner(config *params.ChainConfig, blockNumber *big.Int, blockTime uint64) Signer {
	...
	case config.IsPrague(blockNumber, blockTime):
		if !config.IsOptimism() {
			...
			signer = NewOptimismPragueSigner(config.ChainID)
		}
...
type optimismPragueSigner struct{ londonSigner }

// NewOptimismPragueSigner returns a signer that accepts
// ZRC: optimismSigner is pragueSigner except
// ZRC: londonSigner instead of cancunSigner
// - EIP-7702 set code transactions
// - EIP-1559 dynamic fee transactions
// - EIP-2930 access list transactions,
// - EIP-155 replay protected transactions, and
// - legacy Homestead transactions.
func NewOptimismPragueSigner(chainId *big.Int) Signer {
	return optimismPragueSigner{londonSigner{eip2930Signer{NewEIP155Signer(chainId)}}}
}

func (s optimismPragueSigner) Sender(tx *Transaction) (common.Address, error) {
	if tx.Type() != SetCodeTxType {
...
func (s optimismPragueSigner) Equal(s2 Signer) bool {
	x, ok := s2.(optimismPragueSigner)
	return ok && x.chainId.Cmp(s.chainId) == 0
}

func (s optimismPragueSigner) SignatureValues(tx *Transaction, sig []byte) (R, S, V *big.Int, err error) {
	txdata, ok := tx.inner.(*SetCodeTx)
	if !ok {
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s optimismPragueSigner) Hash(tx *Transaction) common.Hash {
	if tx.Type() != SetCodeTxType {
