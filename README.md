...
## Building image and spinning up local Geth with CCC
First make sure that your machine is x86 architecture. To start compile the circuit capacity checker (CCC):
```
cd rollup/circuitcapacitychecker/libzkp/
cargo build --release
find ./ | grep libzktrie.so | xargs -I{} cp -u {} ./target/release/
cd ../../..
```
Now we build a local image containing CCC:
```
earthly +docker-l2geth-ccc --VERSION_TAG=[YOUR_VERSION]
```
Spin up Geth:
```
docker-compose down -v --remove-orphans && docker-compose -f docker-compose.yaml up geth
```
## Building image witout CCC
```
earthly --push +docker-l2geth-no-ccc --VERSION_TAG=[YOUR_VERSION]
```
## Local L2GETH setup with Devnet
First, make sure that a locally built image is available on registry. For now, Eartly [doesn't](https://github.com/earthly/earthly/issues/823) use local images.

Afterwards, set the `L2GETH_VERSION` with your image in the zkr-monorepo [here](https://github.com/zircuit-labs/zkr-monorepo/blob/develop/Earthfile#L12)

Finally, make sure you set the appropriate platform in the [DOCKERFILE](https://github.com/zircuit-labs/zkr-monorepo/blob/develop/ops-bedrock/Dockerfile.l2#L1) that uses the built image. 

Spin up Devnet using:
```
make devnet-up
```
