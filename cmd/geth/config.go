...
import (
	...
	"github.com/zircuit-labs/l2-geth/accounts/external"
	"github.com/zircuit-labs/l2-geth/accounts/keystore"
	"github.com/zircuit-labs/l2-geth/accounts/scwallet"
	"github.com/zircuit-labs/l2-geth/accounts/usbwallet"
...
func defaultNodeConfig() node.Config {
	...
	cfg.Name = node.DefaultNodeName
	cfg.Version = params.VersionWithMeta
...
func loadBaseConfig(ctx *cli.Context) gethConfig {
	...
	if file := ctx.String(configFileFlag.Name); file != "" {
		...
		// load and start watching SLS config file
		if _, err := sls.InitReader(file, time.Duration(cfg.Eth.SLSConfig.RefreshableInterval)); err != nil {
			utils.Fatalf("failed to load SLS config: %v", err)
		}
	}
...
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend, gethConfig) {
	...
	if ctx.IsSet(utils.OverrideZircuitAlfie.Name) {
		v := ctx.Uint64(utils.OverrideZircuitAlfie.Name)
		cfg.Eth.OverrideZircuitAlfie = &v
	}
	...
	if ctx.IsSet(utils.OverrideZircuitMonofee.Name) {
		v := ctx.Uint64(utils.OverrideZircuitMonofee.Name)
