...
func initGenesis(ctx *cli.Context) error {
	...
	defer func(file *os.File) {
		log.Info("Closing genesis file", "file", genesisPath)
		err := file.Close()
		if err != nil {
			log.Warn("Failed to close genesis file", "err", err)
		}
		log.Info("Genesis file closed", "file", genesisPath)
	}(file)
	...
	defer func(stack *node.Node) {
		log.Info("Closing stack", "stack", stack)
		err := stack.Close()
		if err != nil {
			log.Warn("Failed to close stack", "err", err)
		}
		log.Info("Stack closed", "stack", stack)
	}(stack)
	...
	for _, name := range []string{"chaindata", "lightchaindata"} {
		...
		defer closeDatabase(name, chaindb)

		triedb := utils.MakeTrieDatabase(ctx, chaindb, ctx.Bool(utils.CachePreimagesFlag.Name), false, genesis.IsVerkle())
		defer closeDatabase(name, triedb)
...
type (
	closeableDatabase interface {
		Close() error
	}
)

func closeDatabase(name string, db closeableDatabase) {
	log.Info("Closing database", "name", name)

	if err := db.Close(); err != nil {
		log.Warn("Failed to close database", "name", name, "err", err)
	}

	log.Info("Database closed", "name", name)
}
