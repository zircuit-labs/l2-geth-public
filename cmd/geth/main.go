...
import (
	...
	"github.com/zircuit-labs/l2-geth/console/prompt"
	...
	_ "github.com/zircuit-labs/l2-geth/eth/tracers/js"
	_ "github.com/zircuit-labs/l2-geth/eth/tracers/native"
	"github.com/zircuit-labs/l2-geth/ethclient"
	"github.com/zircuit-labs/l2-geth/internal/debug"
...
const (
	clientIdentifier = "zircuit-l2geth" // Client identifier to advertise over the network
)
...
var (
	...
	nodeFlags = flags.Merge([]cli.Flag{
		...
		utils.OverrideZircuitAlfie,
		utils.OverrideZircuitMonofee,
		...
		utils.MinerTransactionLimitFlag,
		utils.NATFlag,
...
`)
	...
	case ctx.IsSet(utils.ZircuitNetworkFlag.Name):
		log.Info("Starting geth on a Zircuit network...", "network", ctx.String(utils.ZircuitNetworkFlag.Name))
	...
	} else if ctx.String(utils.SyncModeFlag.Name) != "light" && !ctx.IsSet(utils.CacheFlag.Name) && ctx.IsSet(utils.ZircuitNetworkFlag.Name) {
		// We haven't set the cache, but may used the OP network flag we may be on an OP stack mainnet.
		if strings.Contains(ctx.String(utils.ZircuitNetworkFlag.Name), "mainnet") {
			log.Info("Bumping default cache on mainnet", "provided", ctx.Int(utils.CacheFlag.Name), "updated", 4096, "network", ctx.String(utils.ZircuitNetworkFlag.Name))
			ctx.Set(utils.CacheFlag.Name, strconv.Itoa(4096))
		}
	}
	...
	if _, ok := os.LookupEnv("DD_APM_ENABLED"); ok {
		...
		if err != nil {
			log.Info("Failed to start profiler", "error", err)
		}

		tracer.Start(
			tracer.WithService("l2geth"),
		)
	}
}
...
func geth(ctx *cli.Context) error {
	...
	startNode(ctx, stack, backend, false, cfg.Eth)
	stack.Wait()
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend, isConsole bool, cfg ethconfig.Config) {
	// Start up the node itself
	utils.StartNode(ctx, stack, isConsole, cfg)
...
func unlockAccounts(ctx *cli.Context, stack *node.Node) {
	var unlocks []string
	inputs := strings.SplitSeq(ctx.String(utils.UnlockedAccountFlag.Name), ",")
