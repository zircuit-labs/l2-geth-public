...
//go:generate go tool gencodec -type ExecutionResult -field-override executionResultMarshaling -out gen_execresult.go
...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig, txIt txIterator, miningReward int64) (*state.StateDB, *ExecutionResult, []byte, error) {
	...
	if pre.Env.BlockHashes != nil && chainConfig.IsTenrec(pre.Env.Timestamp) {
		var (
	...
	isTenrec := chainConfig.IsTenrec(vmContext.Time)

	// Gather the execution-layer triggered requests.
	var requests [][]byte
	if chainConfig.IsTenrec(vmContext.Time) && !isTenrec {
