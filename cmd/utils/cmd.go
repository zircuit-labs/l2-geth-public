...
import (
	...
	"github.com/zircuit-labs/l2-geth/core/sls-common/slsstray"
...
func StartNode(ctx *cli.Context, stack *node.Node, isConsole bool, cfg ethconfig.Config) {
	// check if it is replica node and SLS data sync flag is enabled
	shouldSyncSLSData := cfg.RollupDisableTxPoolAdmission && cfg.SLSConfig.EnableSLSDataSync
	if shouldSyncSLSData {
		s3Client, err := s3.NewBlobStoreFromConfig(ctx.Context, cfg.SLSConfig.SLSDataSyncS3)
		if err != nil {
			Fatalf("Failed to init s3 client")
		}
		dbPath := stack.ResolvePath(slsstray.DefaultDBPath)

		slsDataSyncer := slsstray.NewSLSDataSyncer(s3Client, dbPath)
		if err := slsDataSyncer.SyncSLSData(ctx.Context); err != nil {
			Fatalf("Failed to sync SLS data: %v", err)
		}
	}
