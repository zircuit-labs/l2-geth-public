...
import (
	...
	"github.com/zircuit-labs/l2-geth/common/fdlimit"
	...
	"github.com/zircuit-labs/l2-geth/ethdb/remotedb"
	"github.com/zircuit-labs/l2-geth/ethstats"
	"github.com/zircuit-labs/l2-geth/graphql"
	...
	"github.com/zircuit-labs/l2-geth/metrics/exp"
	"github.com/zircuit-labs/l2-geth/metrics/influxdb"
	...
	"github.com/zircuit-labs/l2-geth/p2p/nat"
	"github.com/zircuit-labs/l2-geth/p2p/netutil"
	"github.com/zircuit-labs/l2-geth/params"
	"github.com/zircuit-labs/l2-geth/rpc"
	"github.com/zircuit-labs/l2-geth/triedb"
	"github.com/zircuit-labs/l2-geth/triedb/hashdb"
...
var (
	...
	ZircuitNetworkFlag = &cli.StringFlag{
		Name:     "network",
		Usage:    "Select a pre-configured Zircuit network" + strings.Join(core.ZircuitNetworkNames(), ", "),
		Category: flags.EthCategory,
	}
	...
	OverrideZircuitAlfie = &cli.Uint64Flag{
		Name:     "override.alfie",
		Usage:    "Manually specify the Zircuit alfie feature-set fork timestamp, overriding the bundled setting",
		Category: flags.EthCategory,
	}
	OverrideZircuitMonofee = &cli.Uint64Flag{
		Name:     "override.monofee",
		Usage:    "Manually specify the Zircuit MonoFee feature-set fork timestamp, overriding the bundled setting",
		Category: flags.EthCategory,
	}
	...
	MinerTransactionLimitFlag = &cli.Uint64Flag{
		Name:     "miner.txlimit",
		Usage:    "Maximum number of transactions to have in new blocks",
		Category: flags.MinerCategory,
	}
...
var (
	...
	NetworkFlags = append([]cli.Flag{MainnetFlag, ZircuitNetworkFlag}, TestnetFlags...)
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		if ctx.IsSet(ZircuitNetworkFlag.Name) {
			return filepath.Join(path, ctx.String(ZircuitNetworkFlag.Name))
		}
		return path
	}
...
func setBootstrapNodesV5(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.IsSet(ZircuitNetworkFlag.Name):
		network := ctx.String(ZircuitNetworkFlag.Name)
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.IsSet(ZircuitNetworkFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), ctx.String(ZircuitNetworkFlag.Name))
	}
}
...
func setMiner(ctx *cli.Context, cfg *miner.Config) {
	...
	if ctx.IsSet(MinerTransactionLimitFlag.Name) {
		txLimit := ctx.Uint64(MinerTransactionLimitFlag.Name)
		cfg.TransactionLimit = &txLimit
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, GoerliFlag, SepoliaFlag, HoleskyFlag, ZircuitNetworkFlag)
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.IsSet(ZircuitNetworkFlag.Name):
		name := ctx.String(ZircuitNetworkFlag.Name)
		genesis, ok := core.ZircuitNetworksByName[name]
		if !ok {
			Fatalf("Zircuit network %q does not exist", name)
		}
		return genesis
	case ctx.Bool(DeveloperFlag.Name):
