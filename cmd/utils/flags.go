...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/common/fdlimit"
	...
	"github.com/zircuit-labs/l2-geth-public/eth/filters"
	"github.com/zircuit-labs/l2-geth-public/eth/gasprice"
	"github.com/zircuit-labs/l2-geth-public/eth/tracers"
	"github.com/zircuit-labs/l2-geth-public/ethdb"
	"github.com/zircuit-labs/l2-geth-public/ethdb/remotedb"
	"github.com/zircuit-labs/l2-geth-public/ethstats"
	"github.com/zircuit-labs/l2-geth-public/graphql"
	"github.com/zircuit-labs/l2-geth-public/internal/ethapi"
	"github.com/zircuit-labs/l2-geth-public/internal/flags"
	"github.com/zircuit-labs/l2-geth-public/log"
	"github.com/zircuit-labs/l2-geth-public/metrics"
	"github.com/zircuit-labs/l2-geth-public/metrics/exp"
	"github.com/zircuit-labs/l2-geth-public/metrics/influxdb"
	...
	"github.com/zircuit-labs/l2-geth-public/p2p/nat"
...
var (
	...
	CircuitCapacityCheckEnabledFlag = &cli.BoolFlag{
		Name:  "circuit-capacity-check",
		Usage: "Enable circuit capacity check during block validation",
	}

	CircuitCapacityCheckDepositFlag = &cli.BoolFlag{
		Name:  "circuit-capacity-check-deposit",
		Usage: "Enable circuit capacity check for deposit Txs",
	}

	// ZKTrie  settings
	ZKTrieEnabledFlag = &cli.BoolFlag{
		Name:    "zktrie-enabled",
		Usage:   "Enable zktrie and poseidon hashing",
		EnvVars: []string{"ZKTRIE_ENABLED"},
	}
)
...
func setCircuitCapacityCheck(ctx *cli.Context, cfg *ethconfig.Config) {
	if ctx.IsSet(CircuitCapacityCheckEnabledFlag.Name) {
		log.Info(fmt.Sprintf("setting CircuitCapacityCheckEnabledFlag to: %v", ctx.Bool(CircuitCapacityCheckEnabledFlag.Name)))
		cfg.CCCConfig.Enabled = ctx.Bool(CircuitCapacityCheckEnabledFlag.Name)
	}
	if ctx.IsSet(CircuitCapacityCheckDepositFlag.Name) {
		log.Info(fmt.Sprintf("setting CircuitCapacityCheckDepositFlag to: %v", ctx.Bool(CircuitCapacityCheckDepositFlag.Name)))
		cfg.CCCConfig.CheckDeposits = ctx.Bool(CircuitCapacityCheckDepositFlag.Name)
	}
}

func setZKTrieEnabled(ctx *cli.Context) {
	set := ctx.IsSet(ZKTrieEnabledFlag.Name)
	params.ZKTrieEnabled.Store(set)
	if set {
		log.Info("ZKTrie is ENABLED via CLI flag, block config will be ignored")
		return
	}
	log.Trace("ZKTrie is not forced via flag, block will be checked")
}
...
func MakeTrieDatabase(ctx *cli.Context, disk ethdb.Database, preimage, readOnly, isVerkle bool) *trie.Database {
	config := &trie.Config{
		Preimages: preimage,
		IsVerkle:  isVerkle,
		// Do not set IsZkTrie here, instead allowing to override IsZkTrie afterward from *trie.Database
	}
