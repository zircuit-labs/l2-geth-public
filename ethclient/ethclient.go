...
func (ec *Client) BlockByHash(ctx context.Context, hash common.Hash) (*types.Block, error) {
	block, err := ec.getBlock(ctx, "eth_getBlockByHash", hash, true)
...
func (ec *Client) BlockByNumber(ctx context.Context, number *big.Int) (*types.Block, error) {
	block, err := ec.getBlock(ctx, "eth_getBlockByNumber", toBlockNumArg(number), true)
	return block.Block, err
}

// BlockByHashEx returns an extended version of BlockByHash.
func (ec *Client) BlockByHashEx(ctx context.Context, hash common.Hash) (*BlockEx, error) {
	return ec.getBlock(ctx, "eth_getBlockByHashEx", hash, true)
}

// BlockByNumberEx returns an extended version of BlockByNumber.
func (ec *Client) BlockByNumberEx(ctx context.Context, number *big.Int) (*BlockEx, error) {
	return ec.getBlock(ctx, "eth_getBlockByNumberEx", toBlockNumArg(number), true)
}
...
func (ec *Client) getBlock(ctx context.Context, method string, args ...interface{}) (*BlockEx, error) {
...
type DebugBlockTraceResult struct {
	BlockTrace *types.BlockTrace                `json:"blockTrace"`
	CodeMap    map[common.Address]hexutil.Bytes `json:"codeMap"` // map with the code of all addresses involved in the block
	Proofs     []*AccountResult                 `json:"proofs"`  // list with the Proofs for all addresses involved in the block
}

// GetBlockTraceByHash returns the BlockTrace given the block hash.
func (ec *Client) GetBlockTraceByHash(ctx context.Context, blockHash common.Hash) (*DebugBlockTraceResult, error) {
	type storageResult struct {
		Key   string       `json:"key"`
	...
	var res blockTraceResult
	err := ec.c.CallContext(ctx, &res, "zirc_debugBlockTrace", blockHash)

	// Turn hexutils back to normal datatypes
	accountResults := make([]*AccountResult, 0, len(res.Proofs))
	for _, accountRes := range res.Proofs {
		storageResults := make([]StorageResult, 0, len(accountRes.StorageProof))
		for _, st := range accountRes.StorageProof {
		...
		accountResults = append(accountResults, &AccountResult{
			...
			AccountProof:     accountRes.AccountProof,
			Balance:          accountRes.Balance.ToInt(),
			Nonce:            uint64(accountRes.Nonce),
			KeccakCodeHash:   accountRes.KeccakCodeHash,
			PoseidonCodeHash: accountRes.PoseidonCodeHash,
			CodeSize:         uint64(accountRes.CodeSize),
			StorageHash:      accountRes.StorageHash,
			StorageProof:     storageResults,
		})
	}

	result := &DebugBlockTraceResult{
		BlockTrace: res.BlockTrace,
		CodeMap:    res.CodeMap,
		Proofs:     accountResults,
	}
	return result, err
}

// GetRowConsumptionByBlockHashes returns summed up row consumption for each operation by block hashes.
func (ec *Client) GetRowConsumptionByBlockHashes(ctx context.Context, blockHashes []common.Hash) (*types.RowConsumption, error) {
	rowConsumption := new(types.RowConsumption)

	if err := ec.c.CallContext(ctx, &rowConsumption, "eth_getRowConsumptionByBlockHashes", blockHashes); err != nil {
		return nil, err
	}

	return rowConsumption, nil
}
