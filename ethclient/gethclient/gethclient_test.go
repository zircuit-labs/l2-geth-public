...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	config := &ethconfig.Config{Genesis: genesis, TxPool: legacypool.Config{}}
...
func testGetProof(t *testing.T, client *rpc.Client, addr common.Address) {
	...
	if have, want := result.KeccakCodeHash, crypto.Keccak256Hash(code); have != want {
		t.Fatalf("keccakCodeHash wrong, have %v want %v ", have, want)
	}
	if params.ZKTrieEnabled.Load() {
		if have, want := result.PoseidonCodeHash, poseidon.CodeHash(code); have != want {
			t.Fatalf("poseidonCodeHash wrong, have %v want %v ", have, want)
		}
...
func testGetProofNonExistent(t *testing.T, client *rpc.Client) {
	...
	// test codeHash
	if have, want := result.KeccakCodeHash, (types.EmptyKeccakCodeHash); have != want {
