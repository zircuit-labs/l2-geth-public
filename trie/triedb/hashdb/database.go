...
func (db *Database) Update(root common.Hash, parent common.Hash, block uint64, nodes *trienode.MergedNodeSet, states *triestate.Set) error {
	// Ensure the parent state is present and signal a warning if not (unnecessary for ZKTrie).
	// If ZKTrie is globally enabled, we won't call Update for Trie state.
	if !db.isZkTrie && !params.ZKTrieEnabled.Load() && parent != types.EmptyLegacyTrieRootHash {
		if blob, _ := db.Node(parent); len(blob) == 0 {
			log.Warn("parent state is not present", "parent", parent)
		}
	}
	db.lock.Lock()
	defer db.lock.Unlock()
	...
	if set, present := nodes.Sets[common.Hash{}]; present {
		for _, n := range set.Leaves {
			...
			if !types.IsEmptyTrieRoot(account.Root) {
