...
func newTester(t *testing.T, historyLimit uint64) *tester {
	...
	for i := 0; i < 2*128; i++ {
		parent := types.EmptyLegacyTrieRootHash
		if len(obj.roots) != 0 {
...
func (t *tester) generateStorage(ctx *genctx, addr common.Address) common.Hash {
	...
	root, set := updateTrie(addrHash, types.EmptyLegacyTrieRootHash, storage, nil)
...
func (t *tester) generate(parent common.Hash) (common.Hash, *trienode.MergedNodeSet, *triestate.Set) {
	...
	for i := 0; i < 20; i++ {
		...
		case deleteAccountOp:
			...
			acct, _ := types.FullAccount(account)
			if acct.Root != types.EmptyLegacyTrieRootHash {
...
func TestDatabaseRecoverable(t *testing.T) {
	...
	cases := []struct {
		...
		{types.EmptyLegacyTrieRootHash, true},
...
func TestDisable(t *testing.T) {
	...
	if err := tester.db.Enable(types.EmptyLegacyTrieRootHash); err == nil {
