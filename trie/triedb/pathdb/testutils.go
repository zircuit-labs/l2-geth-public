...
func (h *testHasher) Commit(collectLeaf bool) (common.Hash, *trienode.NodeSet, error) {
	...
	if root != types.EmptyLegacyTrieRootHash && root != h.root {
		set.AddNode(nil, trienode.New(root, blob))
	}
	if root == types.EmptyLegacyTrieRootHash && h.root != types.EmptyLegacyTrieRootHash {
...
func hash(states map[common.Hash][]byte) (common.Hash, []byte) {
	...
	if len(input) == 0 {
		return types.EmptyLegacyTrieRootHash, nil
	}
