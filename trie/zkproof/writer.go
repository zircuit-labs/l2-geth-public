...
func NewZkTrieProofWriter(storage *types.StorageTrace) (*zktrieProofWriter, error) {
	...
	zkDb := trie.NewZktrieDatabase(rawdb.NewDatabase(underlayerDb))
	...
	zktrie, err := trie.NewZkTrie(
		storage.RootBefore,
		trie.NewZktrieDatabase(rawdb.NewDatabase(underlayerDb)),
	)
	if err != nil {
...
func getAccountDataFromLogState(state *types.AccountWrapper) *types.StateAccount {
	...
	return &types.StateAccount{
		Nonce:            state.Nonce,
		Balance:          uint256.MustFromBig(state.Balance.ToInt()),
...
func (w *zktrieProofWriter) traceAccountUpdate(addr common.Address, updateAccData func(*types.StateAccount) *types.StateAccount) (*StorageTrace, error) {
	...
	if accDataBefore != nil {
		...
		out.AccountUpdate[0] = &StateAccount{
			...
			Balance:          (*hexutil.Big)(big.NewInt(0).Set(accDataBefore.Balance.ToBig())),
	...
	if accData != nil {
		out.AccountUpdate[1] = &StateAccount{
			...
			Balance:          (*hexutil.Big)(big.NewInt(0).Set(accData.Balance.ToBig())),
	...
	if accData != nil {
		...
		if err := w.tracingZktrie.TryDelete(addr.Bytes()); err != nil {
