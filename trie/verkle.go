...
func NewVerkleTrie(root common.Hash, db *Database, cache *utils.PointCache) (*VerkleTrie, error) {
	...
	node := verkle.New()
	if root != types.EmptyVerkleHash && root != types.EmptyLegacyTrieRootHash {
...
func (t *VerkleTrie) GetAccount(addr common.Address) (*types.StateAccount, error) {
	...
	// Decode codehash
	acc.KeccakCodeHash = values[utils.CodeKeccakLeafKey]
...
func (t *VerkleTrie) UpdateAccount(addr common.Address, acc *types.StateAccount) error {
	...
	values[utils.CodeKeccakLeafKey] = acc.KeccakCodeHash[:]
