...
func TestZktrieDelete(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		{padString("do"), "verb"},
		{padString("ether"), "wookiedoo"},
		{padString("horse"), "stallion"},
		{padString("shaman"), "horse"},
		{padString("doge"), "coin"},
		{padString("ether"), ""},
		{padString("dog"), "puppy"},
		{padString("shaman"), ""},
	...
	hash := trie.Hash()
	exp := common.HexToHash("21ba1176eb47479c60b151a22f32dfb01e1109d2b98b40189323894aa9f6415b")
	if hash != exp {
		t.Errorf("expected %x got %x", exp, hash)
	}
}

// padString left-pads the string with spaces to make it 32 bytes long
// trie requires keys of 20 or 32 bytes only
func padString(s string) string {
	return fmt.Sprintf("%32s", s)
}
...
func tempDBZK(b *testing.B) (string, *Database) {
	...
	assert.NoError(b, err)
	config := &Config{Preimages: true, IsZktrie: true}
