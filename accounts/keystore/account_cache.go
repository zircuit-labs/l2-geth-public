...
func newAccountCache(keydir string) (*accountCache, chan struct{}) {
	ac := &accountCache{
		...
		fileC:  fileCache{all: collections.NewSet[string]()},
	}
...
func (ac *accountCache) deleteByFile(path string) {
	...
	if i < len(ac.all) && ac.all[i].URL.Path == path {
		removed := ac.all[i]
		ac.all = slices0.Delete(ac.all, i, i+1)
		if ba := removeAccount(ac.byAddr[removed.Address], removed); len(ba) == 0 {
			delete(ac.byAddr, removed.Address)
		} else {
			ac.byAddr[removed.Address] = ba
...
func removeAccount(slice []accounts.Account, elem accounts.Account) []accounts.Account {
	for i := range slice {
		if slice[i] == elem {
			return slices0.Delete(slice, i, i+1)
		}
	}
	return slice
}
...
func (ac *accountCache) scanAccounts() error {
	...
	for _, path := range creates.Members() {
		if a := readAccount(path); a != nil {
			ac.add(*a)
		}
	}
	for _, path := range deletes.Members() {
		ac.deleteByFile(path)
	}
	for _, path := range updates.Members() {
		ac.deleteByFile(path)
		if a := readAccount(path); a != nil {
			ac.add(*a)
		}
