...
// https://github.com/zircuit-labs/EIPs/blob/master/EIPS/eip-3675.md
//
...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header) error {
	...
	if !cancun {
		...
		if chain.Config().IsTenrec(header.Time) {
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
	...
	// Assign the final stateDb root to header.
	header.Root = state.IntermediateRoot(true)

	if chain.Config().IsTenrec(header.Time) {
		// State-root has just been computed, we can fetch the withdrawal rooot now.
		h := state.GetState(params.OptimismL2ToL1MessagePasser, params.WithdrawalRootStorageSlot)
		header.WithdrawalsHash = &h
	}
	// Assemble and return the final block.
	return types.NewBlockWithWithdrawals(header, body.Transactions, body.Uncles, receipts, body.Withdrawals, trie.NewStackTrie(nil), chain.Config().IsTenrec(header.Time)), nil
}
