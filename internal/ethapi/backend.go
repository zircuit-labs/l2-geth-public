...
import (
	...
	"github.com/zircuit-labs/l2-geth/core/sls-public/slsapi"
...
func GetAPIs(apiBackend Backend, slsConfig sls.Config) ([]rpc.API, error) {
	...
	slsAPIs, zircAPI, err := slsapi.Factory(ctx, slsConfig, NewPayloadFormatter())
	if err != nil {
		return nil, err
	}
	...
	apis := []rpc.API{
		{
			...
			Namespace: "eth",
			Service:   NewTransactionAPI(apiBackend, nonceLock, zircAPI),
		}, {
			Namespace: "txpool",
	...
	return append(apis, slsAPIs...), nil
}
