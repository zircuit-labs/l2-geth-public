...
import (
	...
	"github.com/zircuit-labs/l2-geth/core/sls-common/storage"
	...
	"github.com/zircuit-labs/l2-geth/internal/blocktest"
...
func TestTransactionBlobTx(t *testing.T) {
	...
	tests := allBlobTxs(common.Address{0xde, 0xad})
...
func allBlobTxs(addr common.Address) []txData {
	return []txData{
		{
			Tx: &types.BlobTx{
				Nonce:      6,
...
func TestCall(t *testing.T) {
	...
	testSuite := []struct {
		...
		// ZRC: We do not support BLOBHASH/BLOBBASEFEE from cancun
		// BLOBHASH opcode
		// {
		// 	blockNumber: rpc.LatestBlockNumber,
		// 	call: TransactionArgs{
...
func TestSignTransaction(t *testing.T) {
	...
	api := NewTransactionAPI(b, nil, slsapiCommon.NewZircDisabledAPI())
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	var (
		...
		pending = types.NewBlockWithWithdrawals(&types.Header{Number: big.NewInt(11), Time: 42}, []*types.Transaction{tx}, nil, nil, []*types.Withdrawal{withdrawal}, blocktest.NewHasher(), false)
	)
...
func TestRPCGetProof(t *testing.T) {
	t.Parallel()

	// Initialize test accounts
	accounts := make([]common.Address, 0, 5)
	for i := range cap(accounts) {
		v := strings.Repeat(strconv.Itoa(i+1), 64)[:64]
		key, err := crypto.HexToECDSA(v)
		require.NoError(t, err, "failed to make key for %q", v)
	...
	var (
		...
		genesis  = &core.Genesis{
			Config: params.TestChainConfig,
			Alloc: types.GenesisAlloc{
				accounts[0]: {Balance: big.NewInt(11111)},
				accounts[1]: {Balance: big.NewInt(22222), Code: code},
				accounts[2]: {Balance: nil},
				accounts[3]: {Balance: nil, Code: code},
			},
		}
	)
	backend := newTestBackend(t, 0, genesis, ethash.NewFaker(), nil)
	api := NewBlockChainAPI(backend)
	blockNumber := rpc.BlockNumber(0)
	...
	testCases := []struct {
		...
		{
			name:     "account with balance and no code",
			address:  accounts[0],
			balance:  (*hexutil.Big)(big.NewInt(11111)),
			codeHash: types.EmptyCodeHash,
		},
		{
			name:     "account with balance and code",
			address:  accounts[1],
			balance:  (*hexutil.Big)(big.NewInt(22222)),
			codeHash: codeHash,
		},
		{
			name:     "account with 0 balance and no code",
			address:  accounts[2],
			balance:  &hexutil.Big{},
			codeHash: types.EmptyCodeHash,
		},
		{
			name:     "account with 0 balance and code",
			address:  accounts[3],
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			result, err := api.GetProof(context.Background(), tt.address, nil, rpc.BlockNumberOrHash{BlockNumber: &blockNumber})
			require.NoError(t, err)
			require.NotNil(t, result)

			assert.Equal(t, tt.address, result.Address)
			assert.Equal(t, tt.balance.String(), result.Balance.String())
			assert.Equal(t, tt.codeHash, result.CodeHash)
		})
	}
}
...
func TestRPCGetTransactionReceipt(t *testing.T) {
	...
	var (
		...
		api               = NewTransactionAPI(backend, new(AddrLocker), slsapiCommon.NewZircDisabledAPI())
	)
...
func TestRPCGetTransactionReceiptSLS(t *testing.T) {
	...
	nonExistentTxHash := common.HexToHash("0x1111111111111111111122222222222222222222222223333333333333333333")

	quarantinedBy := "Test SLS Detector"
	quarantinedReason := "Test SLS Reason"
	...
	tests := []struct {
		...
		txHash common.Hash
		file   string
		setup  func(mockStorage *slsapiCommon.MockQuarantineStorage)
		err    error
	}{
		{
			...
			setup: func(mockStorage *slsapiCommon.MockQuarantineStorage) {
				mockStorage.EXPECT().FindByHash(gomock.Any(), nonExistentTxHash).Return(&model.Quarantine{
					IsReleased:        false,
					TxData:            txData,
			...
			err: NewSLSQuarantineError(quarantinedReason, quarantinedBy),
		},
		{
			name:   "SLS error case",
			txHash: nonExistentTxHash,
			setup: func(mockStorage *slsapiCommon.MockQuarantineStorage) {
				mockStorage.EXPECT().FindByHash(gomock.Any(), nonExistentTxHash).Return(nil, errors.New("can't query SLS"))
			},
			err: NewSLSError(),
		},
		{
			name:   "Valid receipt found - SLS should not be called",
			txHash: txHashes[2],
			file:   "with-logs",
			setup: func(mockStorage *slsapiCommon.MockQuarantineStorage) {
				// No call expected since tx is found
			},
			err: nil,
		},
	}
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockStorage := slsapiCommon.NewMockQuarantineStorage(ctrl)

			zircAPI := slsapiCommon.NewZircAPI(mockStorage)
			api := NewTransactionAPI(backend, new(AddrLocker), zircAPI)
			...
			// If no error is expected, ensure no error occurred
			require.NoError(t, err)

			// Verify the result against the expected file output
			testRPCResponseWithFile(t, i, result, "eth_getTransactionReceipt", tt.file)
		})
	}
}
...
func TestChainlessChainContext(t *testing.T) {
	genesis := &core.Genesis{
		Config: params.MergedTestChainConfig,
		Alloc:  types.GenesisAlloc{},
	}
	b := newTestBackend(t, 1, genesis, beacon.New(ethash.NewFaker()), func(i int, b *core.BlockGen) {
		b.SetPoS() // empty block generator
	})
	testMap := map[uint64]*types.Header{
		10: {
			GasLimit: 12345678,
			GasUsed:  1476322,
			Time:     9876543,
			Extra:    []byte("test header-1"),
		},
		11: {},
		12: {},
		13: {},
	}
	// setup the chain context
	testContext := NewChainlessChainContext(context.Background(), b, testMap)
	...
	tests := []struct {
		...
		{
			name:       "block exists in the cache",
			headerNum:  10,
			headerHash: testMap[10].Hash(),
			wantHeader: true,
		},
		{
			name:       "Another block exists in the cache",
			headerNum:  11,
			headerHash: testMap[11].Hash(),
			wantHeader: true,
		},
		{
			name:       "block doesn't exist in the both cache and backend",
			headerNum:  100,
			headerHash: common.HexToHash("0x2"),
			wantHeader: false,
		},
		{
			name:       "block doesn't exist in backend and hash doesn't match in cache",
			headerNum:  10,
			headerHash: common.HexToHash("0x10"),
			wantHeader: false,
		},
	}

	for _, tt := range tests {
		result := testContext.GetHeader(tt.headerHash, tt.headerNum)
		if tt.wantHeader {
...
func TestVerifySLSQuarantine(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		name          string
		setup         func(verifier *MockSLSQuarantineVerifier)
		expectedError error
	}{
		{
			name: "SLS Disabled",
			setup: func(verifier *MockSLSQuarantineVerifier) {
				verifier.EXPECT().IsQuarantined(gomock.Any(), gomock.Any()).Return(nil, slsapiCommon.ErrSLSDisabled)
			},
			expectedError: nil,
		},
		{
			name: "Error in SLSVerifier",
			setup: func(verifier *MockSLSQuarantineVerifier) {
				verifier.EXPECT().IsQuarantined(gomock.Any(), gomock.Any()).Return(nil, errors.New("some error"))
			},
			expectedError: NewSLSError(),
		},
		{
			name: "No Quarantine",
			setup: func(verifier *MockSLSQuarantineVerifier) {
				verifier.EXPECT().IsQuarantined(gomock.Any(), gomock.Any()).Return(&slsapiCommon.IsQuarantinedResponse{
					IsQuarantined: false,
					Quarantine:    nil,
				}, nil)
			},
		...
		{
			name: "Quarantined with reason",
			setup: func(verifier *MockSLSQuarantineVerifier) {
				verifier.EXPECT().IsQuarantined(gomock.Any(), gomock.Any()).Return(&slsapiCommon.IsQuarantinedResponse{
					IsQuarantined: true,
					Quarantine: &slsapiCommon.Quarantine{
						QuarantinedReason: "Test Reason",
						QuarantinedBy:     "TestUser",
					},
				}, nil)
			},
			expectedError: NewSLSQuarantineError("Test Reason", "TestUser"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockVerifier := NewMockSLSQuarantineVerifier(ctrl)

			tt.setup(mockVerifier)

			api := &TransactionAPI{
				slsQuarantineVerifier: mockVerifier,
			}

			err := api.verifySLSQuarantine(ctx, hash)
...
func TestRPCGetTransactionByHashSLS(t *testing.T) {
	ctrl := gomock.NewController(t)
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:   "Valid transaction found - SLS should be skipped",
			txHash: txHashes[2],
			file:   "no-logs",
			setup: func(mockStorage *slsapiCommon.MockQuarantineStorage) {
				mockStorage.EXPECT().FindByHash(gomock.Any(), txHashes[2]).Return(nil, storage.ErrTransactionNotFound)
			},
			err: nil,
		},
	}
