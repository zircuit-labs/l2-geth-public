...
import (
	...
	"github.com/zircuit-labs/l2-geth-public/internal/blocktest"
...
func TestSignTransaction(t *testing.T) {
	...
	api := NewTransactionAPI(b, nil, NewZircDisabledAPI())
...
func TestRPCGetProof(t *testing.T) {
	...
	for _, tt := range testCases {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.keccakCodeHash, result.KeccakCodeHash)
		})
	}
}
...
func TestRPCGetTransactionReceipt(t *testing.T) {
	...
	var (
		...
		api               = NewTransactionAPI(backend, new(AddrLocker), NewZircDisabledAPI())
	)
...
func TestRPCGetTransactionReceiptSLS(t *testing.T) {
	...
	var backend, txHashes = setupReceiptBackend(t, 6)
	...
	if err != nil {
		t.Errorf("Error marshaling tx %v", err)
	}
	...
	testSuite := []struct {
	...
	}{
		{
			txHash: txHashes[0],
			setup: func(mockStorage *Mockstorage) {
				mockStorage.EXPECT().IsQuarantinedAndScanned(gomock.Any(), txHashes[0]).Return(&model.TransactionResult{
					Quarantined: true,
		...
		},
		{
			txHash: txHashes[1],
			setup: func(mockStorage *Mockstorage) {
				mockStorage.EXPECT().IsQuarantinedAndScanned(gomock.Any(), txHashes[1]).Return(nil, errors.New("can't query SLS"))
			},
			err: NewSLSError(),
		},
		{
			...
			file:   "with-logs",
			setup: func(mockStorage *Mockstorage) {
				mockStorage.EXPECT().IsQuarantinedAndScanned(gomock.Any(), txHashes[2]).Return(&model.TransactionResult{Quarantined: false}, nil)
			},
		},
	}
	...
	for i, tt := range testSuite {
		...
		zircAPI := NewZircAPI(mockStorage)
